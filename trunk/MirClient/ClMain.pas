...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, MMSystem, Forms,
  Dialogs, ExtCtrls, Textures, DXDraws, DirectX, DXClass, DrawScrn, IntroScn,
  PlayScn, Grobal2, DIB, StdCtrls, WIL, HUtil32, EncryptUnit, Actor, Mpeg,
  MapUnit, DWinCtl, ClFunc, magiceff, SoundUtil, clEvent, IniFiles, Registry,
  Spin, ComCtrls, Grids, Menus, Mask, MShare, Share, StrUtils, MD5EncodeStr,
  SoundEngn, ShellApi, tlHelp32, psAPI, PlugIn, GameImages, PathFind, zLibx,
  MPlayer, OleCtrls, GuaJi, ScktComp;
...
type
  TfrmMain = class(TDxForm)
    ...
    procedure DXDrawMouseMove(Sender: TObject; Shift: TShiftState;
      X, Y: Integer);
  ...
  private
    ...
    procedure ClientObjHealthSpellChanged(DefMsg: pTDefaultMessage;
      sData: string);
    ...
    procedure ClientObjInstanceOpenHealth(DefMsg: pTDefaultMessage;
      sData: string);
    ...
    procedure ClientGetRandomDIB(boOpen: Boolean; BitCount: Integer;
      sData: string);
    ...
    procedure ClientObjBuyStoreItemFail(DefMsg: pTDefaultMessage);
    // 摆摊物品购买失败
    ...
    procedure ClientHeroTakeOnItem(nMakeIndex, nWhere: Integer;
      sItemName: string);
    procedure ClientHeroTakeOffItem(nMakeIndex, nWhere: Integer;
      sItemName: string);
  ...
  public
    ...
    procedure DXDrawMouseDown_(Button: TMouseButton; Shift: TShiftState;
      X, Y: Integer);
    procedure DXDrawMouseMove_(Button: TMouseButton; Shift: TShiftState;
      X, Y: Integer);
    ...
    procedure SendMsg(Actor: TActor; wIdent, wParam: Word;
      nParam1, nParam2, nParam3: Integer; sMsg: string);
    procedure SendDelayMsg(Actor: TActor; wIdent, wParam: Word;
      lParam1, lParam2, lParam3: Integer; sMsg: string;
      dwDelay: LongWord);
    procedure SendUpdateDelayMsg(Actor: TActor; wIdent, wParam: Word;
    ...
    procedure SendTakeOffItemToMasterBag(where: Byte; itmindex: Integer;
      itmname: string);
    procedure SendTakeOffItemToHeroBag(where: Byte; itmindex: Integer;
      itmname: string);
    procedure SendItemToHeroBag(where: Byte; itmindex: Integer;
      itmname: string); // 主人包裹到英雄包裹
    procedure SendItemToMasterBag(where: Byte; itmindex: Integer;
      itmname: string); // 英雄包裹到主人包裹

    procedure SendHeroTakeOnItem(where: Byte; itmindex: Integer;
      itmname: string);
    procedure SendHeroTakeOffItem(where: Byte; itmindex: Integer;
      itmname: string);

    procedure SendTakeOnItemFromHeroBag(where: Byte; itmindex: Integer;
      itmname: string); // 从英雄包裹穿装备到主人包裹
    procedure SendTakeOnItemFromMasterBag(where: Byte; itmindex: Integer;
      itmname: string); // 从主人包裹穿装备到英雄包裹
    ...
    procedure SendRepairFirDragon(nType: Byte; itmindex: Integer;
      itmname: string);
    ...
    function TargetInSwordLongAttackRange(ndir: Integer; target: TActor;
      nRange: Integer): Boolean; overload;
...
uses FState, Progress, DirectDraw;
{$R *.DFM}
...
procedure TfrmMain.SendMsg(Actor: TActor; wIdent, wParam: Word;
  nParam1, nParam2, nParam3: Integer; sMsg: string);
var
...
procedure TfrmMain.SendDelayMsg(Actor: TActor; wIdent, wParam: Word;
  lParam1, lParam2, lParam3: Integer; sMsg: string; dwDelay: LongWord);
var
  ProcessMessage: pTProcessMessage;
begin
...
procedure TfrmMain.SendUpdateDelayMsg(Actor: TActor; wIdent, wParam: Word;
  lParam1, lParam2, lParam3: Integer; sMsg: string; dwDelay: LongWord);
var
  ProcessMessage: pTProcessMessage;
  I: Integer;
begin
  ...
  try
    ...
    begin
      ...
      if (ProcessMessage.wIdent = wIdent) and
        (ProcessMessage.nParam1 = lParam1) then
      begin
        m_MsgList.Delete(I);
  ...
  try
    ...
    begin
      if m_MsgList.Count <= I then
        Break;
      ProcessMessage := m_MsgList.Items[I];
      if ProcessMessage.wIdent = wIdent then
      begin
        m_MsgList.Delete(I);
        Dispose(ProcessMessage);
        Continue;
...
begin
  ...
  try
    ...
    begin
      ProcessMessage := m_MsgList.Items[I];
      if (ProcessMessage.dwDeliveryTime <> 0) and
        (GetTickCount < ProcessMessage.dwDeliveryTime) then
      begin
        Inc(I);
        Continue;
      end;
...
begin
  ...
  case ProcessMsg.wIdent of
    ...
    SM_HEROTAKEONITEM:
      ClientHeroTakeOnItem(ProcessMsg.nParam1, ProcessMsg.wParam,
        ProcessMsg.sMsg);
    SM_HEROTAKEOFFITEM:
      ClientHeroTakeOffItem(ProcessMsg.nParam1, ProcessMsg.wParam,
        ProcessMsg.sMsg);
    SM_STARTSERIESPELL_OK:
      UseSerieMagic;
    SM_STARTSERIESPELL_FAIL:
      StopSerieMagic;
  else
    Result := False;
  end;
...
function KeyboardHookProc(Code: Integer; wParam: Longint;
...
begin
  ...
  begin
    if Msg.wParam = VK_TAB then
    begin
      if (g_FocusCret <> nil) and (g_FocusCret.m_btHair < 4) then
      begin
...
begin
  ...
  for I := Low(g_BoxItems) to High(g_BoxItems) do
  begin
    g_BoxItems[I].S.Name := '';
  end;
end;
...
begin
  ...
  try
    ...
    begin
      RootKey := R_MyRootKey;
      if OpenKey(R_MySubKey, True) then
      begin
...
begin
  ...
  begin
    ...
    if ClientOption.sClosePage1 <> '' then
      ShellExecute(frmMain.Handle, nil,
        PChar(string(ClientOption.sClosePage1)), nil, nil, SW_SHOWNORMAL);
    ...
    if ClientOption.sClosePage2 <> '' then
      ShellExecute(frmMain.Handle, nil,
        PChar(string(ClientOption.sClosePage2)), nil, nil, SW_SHOWNORMAL);
...
{$ELSE}
  ...
  if ini <> nil then
  begin
    g_sCurFontName := ini.ReadString('Client Options', 'FontName',
      g_sCurFontName);
    g_boFullScreen := ini.ReadBool('Client Options', 'FullScreen',
      g_boFullScreen);
    g_sServerAddr := ini.ReadString('Client Options', 'ServerIP',
      g_sServerAddr);
    g_nServerPort := ini.ReadInteger('Client Options', 'ServerPort',
      g_nServerPort);
    SCREENWIDTH := ini.ReadInteger('Client Options', 'ScreenWidth',
      SCREENWIDTH);
    SCREENHEIGHT := ini.ReadInteger('Client Options', 'ScreenHeight',
      SCREENHEIGHT);
    g_nInterfaceType := ini.ReadInteger('Client Options', 'InterfaceType',
      g_nInterfaceType);
...
{$IFEND}
  ...
  if (g_ConfigClient.btMainInterface = 1) or (SCREENWIDTH = 1024) then
  begin
...
{$IFEND}
  ...
  begin
    if Pos(g_sFontName, Screen.Fonts.Strings[I]) > 0 then
    begin
  ...
  if (g_ConfigClient.sDomainName <> '') and
    (StringCrc(g_ConfigClient.sDomainName) = g_ConfigClient.nDomainName) then
  begin
    RegInfo.nParam2 := StringCrc
      (EncryStrHex3(DecryptString(g_ConfigClient.sDomainName),
        IntToStr(RegInfo.nParam1)));
  end;
...
begin
  g_PlugInfo.AppHandle := Handle;
  g_MainHandle := Handle;
  if Assigned(g_PlugInfo.HookInitialize) then
  begin
...
{$IFEND}
    ...
    begin
      DMemoTab2.Clear;
      if DComboboxItemType.itemindex <= 0 then
      begin
      ...
      begin
        if (I mod 50 = 0) and (g_boShowVersionBmp) then
        begin
          frmMain.ProcOnIdle;
          Application.ProcessMessages;
        end;
  ...
  if Assigned(g_PlugInfo.HookInitializeEnd) then
  begin
...
begin
  ...
  for I := 33 to 47 do
    templist1.Add(Chr(I)); // 符号
  for I := 48 to 57 do
    templist2.Add(Chr(I)); // 数字
  for I := 58 to 64 do
    templist1.Add(Chr(I)); // 符号
  for I := 65 to 90 do
    templist3.Add(Chr(I)); // 大写字母
  for I := 91 to 96 do
    templist1.Add(Chr(I)); // 符号
  for I := 97 to 122 do
    templist4.Add(Chr(I)); // 小写字母
  for I := 123 to 126 do
    templist1.Add(Chr(I)); // 符号
  if fh then
    templist.Text := templist.Text + templist1.Text;
  if sz then
    templist.Text := templist.Text + templist2.Text;
  if dx then
    templist.Text := templist.Text + templist3.Text;
  if xx then
    templist.Text := templist.Text + templist4.Text;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Inc(g_nVibrationPos);
        if g_nVibrationPos >= Length(g_VibrationValue) then
        begin
          g_nVibrationPos := 0;
  ...
  if boCanDraw then
  begin
    if (g_boShowVersionBmp) then
    begin
      d := g_BackSurface;
      if d <> nil then
      begin
        ImageCanvas.Fill(d.Pixels[0, 0]); // clWhite
        ImageCanvas.Draw((ImageCanvas.Width - d.Width) div 2,
          (ImageCanvas.Height - d.Height) div 2, d);
      end;
    end
    else
    begin
      ...
      begin
        ...
        begin
          ...
          ImageCanvas.Draw(p.X - (d.ClientRect.Right div 2),
            p.Y - (d.ClientRect.Bottom div 2), d.ClientRect, d);
        end;
      end;
      { ------------------------------------------------------------------------------ }
      ...
      begin
        with ImageCanvas do
        begin
          BoldTextOut((SCREENWIDTH - TextHeight(sGameNoticeName))
              div 2 - TextWidth('A') * 6, SCREENHEIGHT - TextHeight('A') * 5,
            sGameNoticeName, GetRGB(150));
          BoldTextOut((SCREENWIDTH - TextWidth(sGameNoticeStr1)) div 2,
            SCREENHEIGHT - TextHeight('A') * 3, sGameNoticeStr1, GetRGB(150));
          BoldTextOut((SCREENWIDTH - TextWidth(sGameNoticeStr1)) div 2,
            SCREENHEIGHT - TextHeight('A') * 2 + TextHeight('A') div 2,
            sGameNoticeStr2, GetRGB(150));
        end;
      end;
      if g_ConnectionStep <> cnsPlay then
        with ImageCanvas do
          TextOut(TextHeight('A'), SCREENHEIGHT - TextHeight('A'), sVERSION,
            clGray);
      { ------------------------------------------------------------------------------ }
      if g_ConfigClient.boShowFullScreen then
      begin
    ...
    try
      ImageCanvas.DrawSurface(ImageCanvas.ClientRect,
        DXDraw.Surface.ClientRect, PByte(ddsd.lpSurface), ddsd.lPitch,
...
begin
  ...
  if g_boBagLoaded then
    Savebags('.\Data\' + g_sServerName + '.' + g_sSelChrName + '.itm',
      @g_ItemArr);
  g_boBagLoaded := False;
end;
...
begin
  if mrOk = FrmDlg.DMessageDlg('Are you sure you want to log-out?',
    [mbOk, mbCancel]) then
  begin
    DScreen.ClearHint;
    Logout;
...
begin
  if mrOk = FrmDlg.DMessageDlg('Are you sure you want to exit the game?',
    [mbOk, mbCancel]) then
  begin
    if g_boBagLoaded then
      Savebags('.\Data\' + g_sServerName + '.' + g_sSelChrName + '.itm',
...
procedure TfrmMain.PrintScreenNow;
  ...
  procedure BoldOut(Surface: TDirectDrawSurface; X, Y, FColor, Bcolor: Integer;
    Str: string); // 输出抖动加粗
  begin
    with Surface do
    begin
...
begin
  ...
  begin
    ...
    if not FileExists(sDirectory + flname) then
    begin
      DScreen.AddChatBoardString('[Screenshot: ' + flname + '] saved.',
        clGreen, clWhite);
      Break;
    end;
  ...
  if g_MySelf <> nil then
  begin
    BoldOut(DXDraw.Surface, 1, 1, clWhite, clBlack,
      g_sServerName + ' ' + g_MySelf.m_sUserName);
    // DXDraw.Surface.Canvas.TextOut(0, 0, g_sServerName + ' ' + g_MySelf.m_sUserName);
    Inc(n, 1);
  end;
  BoldOut(DXDraw.Surface, 1, (n) * 12 + 1, clWhite, clBlack,
    'CheckSum=' + IntToStr(CheckSum));
  BoldOut(DXDraw.Surface, 1, (n + 1) * 12 + 1, clWhite, clBlack,
    DateToStr(Date));
  BoldOut(DXDraw.Surface, 1, (n + 2) * 12 + 1, clWhite, clBlack,
    TimeToStr(Time));
    ...
    begin
      ...
      case g_ChrAction of
        ...
        caRun:
          ...
          LB_RUN :
            stdcount := 1;
            if g_nRunReadyCount >= stdcount then
            begin
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                bowalk := True;
                bostop := False;
                if not PlayScene.CanRun(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY,
                  mx, my) then
                begin
                  bowalk := False;
                  mdir := 0;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (g_ItemArr[I].S.Name <> '') and (g_ItemArr[I].S.StdMode = 25) and
        (g_ItemArr[I].S.Shape = nType) and
        (Round(g_ItemArr[I].Dura / 100) >= nCount) then
      begin
...
begin
  ...
  if Assigned(g_PlugInfo.HookKeyDown) then
  begin
    if g_PlugInfo.HookKeyDown(Key, Shift) then
      Exit;
  end;
  ...
  case Key of
    VK_HOME, { VK_MULTIPLY, } VK_F12:
      begin
        FrmDlg.DOptionClick();
      end;
  end;

  if g_Config.boUseHotkey then
  begin
    if (g_Config.nSerieSkill <> 0) then
    begin
      nKey := GetInputKey(g_Config.nSerieSkill, AShift);
      if (nKey = Key) and (AShift = Shift) then
      begin
        StartSerieMagic();
    ...
    if (g_Config.nHeroSetAttackState <> 0) then
    begin
    ...
    if g_Config.nHeroCallHero <> 0 then
    begin
      ...
      begin
        ...
        begin
          frmMain.SendClientMessage(CM_HEROLOGOUT, g_MyHero.m_nRecogId, 0, 0,
            0);
        end;
        Key := 0;
      end;
    end;
    if g_Config.nHeroSetTarget <> 0 then
    begin
    ...
    if g_Config.nHeroUnionHit <> 0 then
    begin
    ...
    if g_Config.nHeroSetGuard <> 0 then
    begin
    ...
    if g_Config.nSwitchAttackMode <> 0 then
    begin
    ...
    if g_Config.nSwitchMiniMap <> 0 then
    begin
      ...
      begin
        ...
        if not FrmDlg.DMiniMap.Visible then
        begin
  ...
  case Key of
    VK_F1, VK_F2, VK_F3, VK_F4, VK_F5, VK_F6, VK_F7, VK_F8:
      begin
        if g_ServerConfig.boChgSpeed then
        begin
        ...
        if (GetTickCount - g_dwLatestSpellTick > (dwSpellTime
            { 500 } + g_dwMagicDelayTime)) and (ActionKey = 0) then
        begin
          ...
          case ActionKey of
            ...
            12 .. 19:
              begin
                Magic := GetMagicByKey
                  (Char((ActionKey - 12) + Byte('1') + Byte($14)));
              end;
          end;

          if Magic <> nil then
          begin
            if not IsWarrSkill(Magic.Def.wMagicId) then
            begin
              if (g_MySelf.m_btHorse > 0) and g_Config.boAutoHorse then
                SendOnHorse();
            end;
          end;

          if g_MySelf.m_btJob = 2 then
          begin // 毒符互换
            if Magic <> nil then
            begin
              ...
              if g_Config.boChangeSign and (g_WaitingUseItem.Item.S.Name = '')
                then
              begin
                case Magic.Def.wMagicId of // 自动换毒
                  6:
                    begin
                      if g_Config.boChangePoison then
                      begin
                        if g_nPoisonIndex < 0 then
                          g_nPoisonIndex := g_Config.nPoisonIndex + 1;
                        if (g_UseItems[U_BUJUK].S.Name = '') or
                          (g_UseItems[U_BUJUK].S.StdMode <> 25) or
                          (g_UseItems[U_BUJUK].S.Shape <> g_nPoisonIndex) then
                        begin
                          TakeOnItem(g_nPoisonIndex, 1);
                          Inc(g_nPoisonIndex);
                          if g_nPoisonIndex > 2 then
                            g_nPoisonIndex := 1;
                  ...
                  13, 14, 15, 16, 17, 18, 19, 30, 57, 72, 76:
                    begin // 自动换符
                      nCount := 1;
                      if (Magic.Def.wMagicId = 30) or (Magic.Def.wMagicId = 72)
                        then
                        nCount := 5;
                      if (g_UseItems[U_BUJUK].S.Name = '') or
                        (g_UseItems[U_BUJUK].S.StdMode <> 25) or
                        (g_UseItems[U_BUJUK].S.Shape <> 5) then
                      begin
                        TakeOnItem(5, nCount);
                        // g_dwProcessKeyTick := GetTickCount;
                      end
                      ...
                      begin
                        if (Round(g_UseItems[U_BUJUK].Dura / 100) < nCount) then
                        begin
                          TakeOnItem(5, nCount);
                          // g_dwProcessKeyTick := GetTickCount;
                        end;
                  ...
                  52:
                    begin // 自动换符 诅咒术
                      if (g_UseItems[U_BUJUK].S.Name = '') or
                        (g_UseItems[U_BUJUK].S.StdMode <> 25) or
                        (g_UseItems[U_BUJUK].S.Shape <> 3) then
                      begin
                        TakeOnItem(3, 3);
                        // g_dwProcessKeyTick := GetTickCount;
                      end
                      ...
                      begin
                        if (Round(g_UseItems[U_BUJUK].Dura / 100) < 3) then
                        begin
                          TakeOnItem(3, 3);
                          // g_dwProcessKeyTick := GetTickCount;
                        end;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              DScreen.AddChatBoardString('You cannot leave the Battle?..',
                clyellow, clRed);
          end;
        end;
      end;
            ...
            q_QueryHum:
              begin
                ...
                begin
                  ...
                  DScreen.AddChatBoardString('Reloading Bag.', clWhite,
                    clFuchsia);
                end;
              end;
            q_QueryHero:
              begin
                ...
                begin
                  ...
                  DScreen.AddChatBoardString('Reloading Hero Bag.', clWhite,
                    clFuchsia);
                end
                else
                begin
    ...
    Word('X'):
      begin
        ...
        begin
          ...
          if (GetTickCount - g_dwLatestStruckTick > 10000) and
            (GetTickCount - g_dwLatestMagicTick > 10000) and
            (GetTickCount - g_dwLatestHitTick > 10000) or (g_MySelf.m_boDeath)
            then
          begin
            AppLogout;
          end
    ...
    Word('P'):
      begin
        if ssAlt in Shift then
        begin
          if Assigned(g_PlugInfo.MediaPlayer.Player) then
          begin
...
begin
  ...
  if Assigned(g_PlugInfo.HookKeyPress) then
  begin
    if g_PlugInfo.HookKeyPress(Key) then
      Exit;
  end;
  ...
  begin
    case Byte(Key) of
      ...
      Byte(' '), 13:
        begin
          if FrmDlg.EdChat.Visible and (not FrmDlg.EdChat.Focused) then
          begin
            FrmDlg.EdChat.SetFocus;
            Exit;
          end;
        ...
        begin
          ...
          begin
            if WhisperName = '' then
              ...
              FrmDlg.EdChat.Text := '/' + WhisperName + ' '
            else
              FrmDlg.EdChat.Text := Key;
            FrmDlg.EdChat.SelStart := Length(FrmDlg.EdChat.Text);
            FrmDlg.EdChat.SelLength := 0;
...
begin
  Result := nil;
  for I := 0 to g_SerieMagicList.Count - 1 do
  begin
    pm := PTClientMagic(g_SerieMagicList[I]);
    if pm.Def.wMagicId = wMagicId then
    begin
      Result := pm;
      Break;
    end;
...
var
  ...
  nCount: Integer;
begin
  if (pcm = nil) or g_MySelf.m_boStartStore then
    Exit;
  if (g_NewStatus > sNone) then
    Exit; // 骑马不允许使用魔法
  ...
  begin
    if (pcm.Def.btEffectType = 0) { and (pcm.Def.wMagicId <> 60) } then
    begin
      // if pcm.Def.wMagicId in [3, 7] then Exit;
      if pcm.Def.wMagicId = 26 then
      begin // 烈火
        if GetTickCount - g_dwLatestFireHitTick <
          g_ServerConfig.nFireDelayTime then
        begin
          Exit;
        end;
      ...
      begin // 开天斩
        if GetTickCount - g_dwLatestKTZHitTick <
          g_ServerConfig.nKTZDelayTime then
        begin
          Exit;
        end;
      ...
      if pcm.Def.wMagicId = 43 then
      begin // 破空剑
        if GetTickCount - g_dwLatestPKJHitTick <
          g_ServerConfig.nPKJDelayTime then
        begin
          Exit;
        end;
        g_dwAutoPKJTick := GetTickCount;
      end;
      if pcm.Def.wMagicId = 77 then
      begin // 逐日剑法
        if GetTickCount - g_dwLatestZRJFHitTick <
          g_ServerConfig.nZRJFDelayTime then
        begin
          Exit;
        end;
      ...
      if GetTickCount - g_dwLatestSpellTick > dwSpellTime { 500 } then
      begin
        g_dwLatestSpellTick := GetTickCount;
    ...
    begin
      if (g_MySelf.m_btHorse > 0) or (g_NewStatus > sNone) then
        Exit;

      tdir := GetFlyDirection(390, 175, tx, ty);

      // Magic Lock Code
      if (pcm.Def.wMagicId in [2, 14, 15, 19, 9, 10, 22, 23, 29, 33, 46, 49,
        40, 20, 44, 45, 54, 56, 57, 60, 61, 62, 63, 64, 65]) then
      begin
      ...
      begin
        if g_Config.boMagicLock then
        begin
          if PlayScene.IsValidActor(g_MagicLockActor) then
          begin
            if g_MagicLockActor.m_boDeath then
              g_MagicLockActor := nil;
          end
          else
            g_MagicLockActor := nil;
        end;
        if not g_Config.boMagicLock or (PlayScene.IsValidActor(g_FocusCret) and
            (not g_FocusCret.m_boDeath)) or (g_MagicLockActor = nil) then
        begin
      ...
      begin
        ...
        begin
          ...
          g_dwMagicDelayTime := g_dwMagicDelayTime - Trunc
            ((g_MySelf.m_Abil.AttackSpeed * g_dwMagicDelayTime) / 100);
          {
        ...
        if g_MagicTarget <> nil then
          if g_MagicTarget.m_btRace = 0 then
            g_dwMagicPKDelayTime := g_Config.dwMagicPKDelayTime;
        ...
        if ((g_MagicTarget = nil) or (g_MagicTarget.m_btRace = 0) or
            (g_MagicTarget.m_btRace = RCC_MERCHANT)) then
        begin
          PlayScene.CXYfromMouseXY(tx, ty, targx, targy);
        ...
        begin
          ...
          begin
            PlayScene.CXYfromMouseXY(tx, ty, targx, targy);
            g_MySelf.SendMsg(CM_SPELL, targx, targy, tdir, Integer(pmag), 0,
              '', 0);
          end
          else
          begin
    ...
    DScreen.AddSysMsg('Not enough MP....', 30, 40, clAqua);
end;
...
begin
  ...
  if Actor <> nil then
  begin
    Actor.SendMsg(SM_MAGICFIRE, target
      { 111magid } , efftype, effnum, targetx, targety, '', sound);
...
var
  I: Integer;
begin
  if (g_MySelf = nil) or g_MySelf.m_boDeath or g_boSerieMagic or
    g_boSerieMagicing or (g_SerieMagicList.Count <= 1) or
    (g_MySelf.m_btHorse > 0) or g_MySelf.m_boStartStore or (ActionKey <> 0) or
    (g_NewStatus > sNone) or (not ServerAcceptNextAction) then
    Exit; // {or (not (CanNextAction and ServerAcceptNextAction))}
  g_SerieMagicingList.Clear;
  for I := 0 to 7 do
  begin
    if g_SerieMagic[I].nMagicID >= 0 then
    begin
  ...
  if g_SerieMagicingList.Count >= 2 then
  begin
    if g_SerieTarget = nil then
    begin
      if PlayScene.IsValidActor(g_TargetCret) and (not g_TargetCret.m_boDeath)
        then
      begin
    ...
    if g_SerieTarget = nil then
    begin
      if PlayScene.IsValidActor(g_FocusCret) and (not g_FocusCret.m_boDeath)
        then
      begin
      ...
      begin
        if PlayScene.IsValidActor(g_MagicLockActor) and
          (not g_MagicLockActor.m_boDeath) then
        begin
    ...
    if (not PlayScene.IsValidActor(g_SerieTarget))
      or g_SerieTarget.m_boDeath then
    begin
      g_SerieTarget := nil;
    end;
    ...
    begin
      ...
      g_nSerieIndex := 0;
      g_boSerieMagic := True;
      SendClientMessage(CM_STARTSERIESPELL, g_SerieTarget.m_nRecogId,
        g_SerieTarget.m_nCurrX, g_SerieTarget.m_nCurrY, 0);
    end;
  end;
end;
...
begin
  ...
  if (g_MySelf = nil) or g_MySelf.m_boDeath or (g_MySelf.m_btHorse > 0)
    or g_MySelf.m_boStartStore or (ActionKey <> 0) or (g_NewStatus > sNone)
    or (g_nSerieIndex < 0) or (g_nSerieIndex >= g_SerieMagicingList.Count) or
    (g_SerieMagicList.Count <= 1) then
  begin
    StopSerieMagic();
    Exit;
  end;
  ...
  if g_SerieTarget = nil then
  begin
    if PlayScene.IsValidActor(g_FocusCret) and (not g_FocusCret.m_boDeath) then
    begin
      g_SerieTarget := g_FocusCret;
      g_MagicLockActor := g_FocusCret;
    end
  ...
  if (not PlayScene.IsValidActor(g_SerieTarget)) or g_SerieTarget.m_boDeath then
  begin
    g_SerieTarget := nil;
  end;
  ...
  begin
    ...
    g_nCurrenMagic := 0;
    if Magic.Def.wMagicId in [100 .. 103] then
    begin
    ...
    tdir := GetNextDirection(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY,
      g_SerieTarget.m_nCurrX, g_SerieTarget.m_nCurrY);
    g_MySelf.SendMsg(CM_SPELL, nTargetX, nTargetY, tdir, Integer(pmag),
      nTarget, '', 0);
  end
  else
  begin
    if g_nSerieError < 3 then
    begin
      g_nSerieError := 0;
...
begin
  ...
  begin
    ...
    if (g_EatingItem.S.Name = '') and (g_ItemArr[idx].S.Name <> '') and
      (g_ItemArr[idx].S.StdMode <= 3) then
    begin
      g_EatingItem := g_ItemArr[idx];
      g_ItemArr[idx].S.Name := '';
      if (g_ItemArr[idx].S.StdMode = 4) and (g_ItemArr[idx].S.Shape < 100) then
      begin
        if g_ItemArr[idx].S.Shape < 50 then
        begin
          if mrYes <> FrmDlg.DMessageDlg
            ('Would you like to learn ' + g_EatingItem.S.Name + '?',
            [mbYes, mbNo]) then
          begin
        ...
        begin
          if mrYes <> FrmDlg.DMessageDlg
            ('Will you use' + g_EatingItem.S.Name + '?', [mbYes, mbNo]) then
          begin
            g_ItemArr[idx] := g_EatingItem;
            Exit;
          end;
      ...
      if (g_ItemArr[idx].S.StdMode = 49)
      { and (g_ItemArr[idx].Dura >= g_ItemArr[idx].DuraMax) } then
      begin
        if mrYes <> FrmDlg.DMessageDlg
          ('Will you release ' + g_EatingItem.S.Name + ' experience?',
          [mbYes, mbNo]) then
        begin
          g_ItemArr[idx] := g_EatingItem;
          Exit;
...
begin
  if idx in [0 .. g_MyHero.m_nBagCount] then
  begin
    if (g_HeroEatingItem.S.Name <> '') and
      (GetTickCount - g_dwHeroEatTime > 5 * 1000) then
    begin
      g_HeroEatingItem.S.Name := '';
    end;
    if (g_HeroEatingItem.S.Name = '') and (g_HeroItemArr[idx].S.Name <> '') and
      (g_HeroItemArr[idx].S.StdMode <= 3) then
    begin
      ...
      begin
        ...
        begin
          if mrYes <> FrmDlg.DMessageDlg
            ('Would you like your Hero to learn "' + g_HeroEatingItem.S.Name +
              '"?', [mbYes, mbNo]) then
          begin
        ...
        begin
          if mrYes <> FrmDlg.DMessageDlg
            ('Would you like your Hero to learn  "'
              + g_HeroEatingItem.S.Name + '"?', [mbYes, mbNo]) then
          begin
            g_HeroItemArr[idx] := g_HeroEatingItem;
            Exit;
          end;
...
begin
  ...
  begin
    if (Actor.m_btRace <> RCC_MERCHANT) and (Actor <> g_MyHero) then
    begin
      Result := True;
      ...
      begin
        ...
        if CompareText(sUserName, g_MySelf.m_sUserName) = 0 then
          Result := False;
        if (g_MyHero <> nil) and (CompareText(sUserName,
            g_MyHero.m_sUserName) = 0) then
          Result := False;
      end;
    end;
  end;
...
begin
  ...
  if (target.m_nCurrX = nX) and (target.m_nCurrY = nY) then
  begin
...
function TfrmMain.TargetInSwordLongAttackRange(ndir: Integer; target: TActor;
...
begin
  ...
  if nRange in [2, 3] then
  begin
    if nRange = 3 then
      GetFrontPosition(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, ndir, nX, nY);

    GetFrontPosition(nX, nY, ndir, nX, nY);
    if (abs(g_MySelf.m_nCurrX - nX) = 2) or (abs(g_MySelf.m_nCurrY - nY) = 2)
      then
    begin
      Actor := PlayScene.FindActorXY(nX, nY);
...
begin
  ...
  Result := (Actor <> nil) and (ractor <> nil) and (Actor <> ractor)
    and IsAttackTarget(Actor) and IsAttackTarget(ractor);
end;
...
begin
  ...
  begin
    ...
    Actor := PlayScene.FindActorXY(nX, nY);
    if (Actor <> nil) and IsAttackTarget(Actor) then
      Inc(n12);
    Inc(nC);
    if nC >= 7 then
      Break;
...
procedure TfrmMain.DXDrawMouseMove_(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
...
begin
  ...
  if (g_MySelf = nil) or (DScreen.CurrentScene <> PlayScene) or
    (g_NewStatus > sNone) then
    Exit;
  ...
  begin
    ...
    PlayScene.ScreenXYfromMCXY(g_FocusItem.X, g_FocusItem.Y, mx, my);
    DScreen.ShowHint(mx - 20, my - 10, itemnames, // PTDropItem(ilist[i]).Name,
      clWhite, True);
  end
  else
  begin
    if not g_boClearFocusItemMsg then
    begin
...
procedure TfrmMain.DXDrawMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
var
  Button: TMouseButton;
  pt: TPoint;
...
var
  pm: PTClientMagic;
begin
  if not g_boNextTimePowerHit then
  begin
...
begin
  ...
  Result := CM_HIT;
  if g_boNextTime60Hit and (g_MySelf.m_Abil.MP >= 7) then
  begin
    g_boNextTime60Hit := False;
    Result := CM_60HIT; // 破魂
  end
  else if g_boNextTimeZRJFHit and (g_MySelf.m_Abil.MP >= 7) then
  begin
  ...
  else if g_boNextTimeKTZHit and (g_MySelf.m_Abil.MP >= 7) then
  begin
    g_boNextTimeKTZHit := False;
    Result := CM_KTHIT; // 开天斩
  end
  else if g_boNextTimePKJHit and (g_MySelf.m_Abil.MP >= 7) then
  begin
    g_boNextTimePKJHit := False;
    Result := CM_PKHIT; // 破空剑
  end
  else if (Random(50) = 0) and GetCanPowerHit then
  begin
    Result := CM_POWERHIT; // 攻杀
  end
  else if g_boCanCrsHit and (g_MySelf.m_Abil.MP >= 6)
    and TargetInSwordCrsAttackRange(tdir) then
  begin // 抱月
    Result := CM_CRSHIT;
  end
  else if g_boCanWideHit and (g_MySelf.m_Abil.MP >= 3)
    and TargetInSwordWideAttackRange(tdir) then
  begin
  ...
  if (Result = CM_FIREHIT) or (Result = CM_KTHIT) or (Result = CM_PKHIT) or
    (Result = CM_60HIT) or (Result = CM_ZRJFHIT) or (Result <> CM_HIT) then
    Exit;

  if g_Config.boSmartCrsHit and (g_MySelf.m_Abil.MP >= 6)
    and TargetInSwordCrsAttackRange(tdir) and GetCanCrsHit then
  begin
    Result := CM_CRSHIT; // 抱月
    Exit;
  end;

  if g_Config.boSmartWideHit and (g_MySelf.m_Abil.MP >= 3)
    and TargetInSwordWideAttackRange(tdir) and GetCanWideHit then
  begin
    Result := CM_WIDEHIT; // 半月
    Exit;
  end;

  if (Random(50) = 0) and GetCanPowerHit then
  begin
    Result := CM_POWERHIT; // 攻杀
    Exit;
  end;

  if g_boCanWideHit then
  begin
    Result := CM_WIDEHIT; // 半月
    Exit;
  end;

  if g_boCanCrsHit then
  begin
    Result := CM_CRSHIT; // 抱月
    Exit;
  end;
...
begin
  ...
  while True do
  begin
    if (nCurrX = nTargetX) and (nCurrY = nTargetY) then
      Break;
    ...
    if GetNextDirection(nCurrX, nCurrY, nTargetX, nTargetY) <> btDir then
    begin
      Result := False;
      Break;
    end;
...
begin
  ...
  begin
    ...
    if GetNextDirection(nCurrX, nCurrY, target.m_nCurrX, target.m_nCurrY)
      <> btDir then
    begin
      Result := False;
      Break;
...
begin
  Result :=
  { (nHit = CM_60HIT) or } (nHit = CM_ZRJFHIT) or (nHit = CM_KTHIT) or
    (nHit = CM_PKHIT);
end;
...
function GetWalkLongHitXY(nCurrX, nCurrY: Integer;
  var nX, nY: Integer): Boolean;
var
  btDir: Byte;
  ndir: Integer;
begin
  ...
  nX := nCurrX;
  nY := nCurrY;
  btDir := GetNextDirection(nCurrX, nCurrY, g_MySelf.m_nCurrX,
    g_MySelf.m_nCurrY);
  GetFrontPosition(nX, nY, btDir, 2, nX, nY);
  if ((nCurrX <> nX) or (nCurrY <> nY)) then
  begin
    if (abs(g_MySelf.m_nCurrX - nX) <= 1) and
      (abs(g_MySelf.m_nCurrY - nY) <= 1) then
    begin
  ...
  // ==============================================================================
  ndir := GetNextDirection(nCurrX, nCurrY, g_MySelf.m_nCurrX,
    g_MySelf.m_nCurrY);
  Inc(ndir);
  if ndir > 7 then
    ndir := 0;
...
begin
  ...
  if (not target.m_boDeath) and TargetInSwordLongAttackRange(tdir, target, 1)
    then
  begin
    if CanNextAction and ServerAcceptNextAction and CanNextHit then
    begin
      // nHitMsg := SearchHitMsg(tdir);
      if g_boSerieMagicing and (nHitMsg = CM_101HIT) then
      begin
        SendSpellMsg(CM_SPELL, tdir, 0, 101, target.m_nRecogId);
        // g_SerieTarget.m_nCurrX, g_SerieTarget.m_nCurrY
      end
      else
      begin
  ...
  if (abs(g_MySelf.m_nCurrX - target.m_nCurrX) <= 2) and
    (abs(g_MySelf.m_nCurrY - target.m_nCurrY) <= 2) and
    (not target.m_boDeath) then
    g_ChrAction := caWalk
  else
    g_ChrAction := caRun; // ???
...
procedure TfrmMain.DXDrawMouseDown_(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
...
begin
  ...
  begin // 鼠标右键
    ...
    if g_MySelf.m_boStartStore then
      Exit; // 摆摊状态禁止走动，

    // 按鼠标右键，并且鼠标指向空位置
    PlayScene.CXYfromMouseXY(X, Y, g_nMouseCurrX, g_nMouseCurrY);
    if (abs(g_MySelf.m_nCurrX - g_nMouseCurrX) < 2) and
      (abs(g_MySelf.m_nCurrY - g_nMouseCurrY) < 2) then
    begin // 目标座标
      tdir := GetNextDirection(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY,
  ...
  begin // 按鼠标左键
    ...
    begin
      // 骑马不允许操作
      if PlayScene.IsSelectMyself(X, Y) and (g_MySelf <> nil) and
        (not g_MySelf.m_boDeath) and g_MySelf.m_boStartStore then
      begin // 查询摆摊物品
        if GetTickCount - g_dwQueryStoreTick > 1000 then
        begin
          g_dwQueryStoreTick := GetTickCount;
          SendClientMessage(CM_QUERYSTORE, g_MySelf.m_nRecogId,
            g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, 0);
    ...
    if (g_UseItems[U_WEAPON].S.Name <> '') and (target = nil) and
      (g_MySelf.m_btHorse = 0) and (not g_MySelf.m_boStartStore)
    { 摆摊状态 } and (g_NewStatus = sNone) then
    begin // 骑马状态不可以操作
      // 挖矿
    ...
    if (ssAlt in Shift) and (g_MySelf.m_btHorse = 0) and
      (not g_MySelf.m_boStartStore) and (g_NewStatus = sNone) { 摆摊状态 } then
    begin // 骑马状态不可以操作
      // 挖物品
      tdir := GetNextDirection(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY,
        g_nMouseCurrX, g_nMouseCurrY);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if (target.m_btRace = RCC_MERCHANT) and
              (not g_MySelf.m_boStartStore) { 摆摊状态 } then
            begin
              SendClientMessage(CM_CLICKNPC, target.m_nRecogId, 0, 0,
                BoolToInt(FrmDlg.DMerchantBigDlg.Visible));
              Exit;
            end;
          end;
          if (not target.m_boDeath) and (target.m_btRace = 0)
            and target.m_boStartStore then
          begin // 查询摆摊物品
            if GetTickCount - g_dwQueryStoreTick > 1000 then
            begin
              g_dwQueryStoreTick := GetTickCount;
              SendClientMessage(CM_QUERYSTORE, target.m_nRecogId,
                target.m_nCurrX, target.m_nCurrY, 0);
              // DScreen.AddChatBoardString('SendClientMessage(CM_QUERYSTORE', clRed, clBlue);
            end;
            Exit;
          end;
          if (not target.m_boDeath) and (g_MySelf.m_btHorse = 0) and
            (not g_MySelf.m_boStartStore)
          { 摆摊状态 } and (g_NewStatus = sNone) then
          begin // 骑马不允许操作
            g_TargetCret := target;
            ...
            if (g_NewStatus = sNone) and
              ((target.m_btRace <> RCC_USERHUMAN) and
                (target.m_btRace <> RCC_GUARD) and (target.m_btRace <> 12)
                and // Guard
                (target.m_btRace <> 45) and // Archer
                (target.m_btRace <> RCC_MERCHANT) and (Pos('(',
                  target.m_sUserName) = 0)) or
              ((ssShift in Shift) or ((target.m_btRace <> RCC_MERCHANT))
                and g_Config.boNotNeedShift) // SHIFT + 鼠标左键
              or (target.m_nNameColor = ENEMYCOLOR) or
              (GetRangeHit(nHitMsg) and IsInRangeLine(target.m_nCurrX,
                target.m_nCurrY, 4)) then
            begin
              AttackTarget(target);
              g_dwLatestHitTick := GetTickCount;
            end;
        ...
        begin
          if (g_MySelf.m_btHorse = 0) and (not g_MySelf.m_boStartStore)
          { 摆摊状态 } and (g_NewStatus = sNone) then
          begin
            tdir := GetNextDirection(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY,
              g_nMouseCurrX, g_nMouseCurrY);
      ...
      begin
        if g_MySelf.m_boStartStore { 摆摊状态 } then
          Exit;
        ...
        else if g_ServerConfig.boChgSpeed then
        begin
        ...
        if GetTickCount - g_dwLastAttackTick > dwMoveTime then
...
begin
  ...
  begin
    if g_NewStatus = sConfusion then
    begin // 混乱状态
      if GetTickCount - g_dwConfusionTick > 400 then
      begin
        ...
        if PlayScene.CanWalk(nX, nY) then
        begin
          if (abs(g_nNewStatusX - nX) <= 2) and (abs(g_nNewStatusY - nY) <= 2)
            then
          begin
            if (abs(g_MySelf.m_nCurrX - nX) <= 1) and
              (abs(g_MySelf.m_nCurrY - nY) <= 1) then
  ...
  begin
    if (g_NewStatus > sNone) then
      StopSerieMagic;
    if (g_SerieTarget <> nil) and PlayScene.IsValidActor(g_SerieTarget) and
      (not g_SerieTarget.m_boDeath) then
    begin
      if ActionKey > 0 then
      begin
        ProcessKeyMessages;
      ...
      begin
        ...
        if (GetTickCount - g_dwSerieMagicTick < 10000) then
        begin
          // DScreen.AddChatBoardString('g_boSerieMagicing:2', clyellow, clRed);
          if (g_nCurrenMagic >= CM_100HIT) and (g_nCurrenMagic <= CM_103HIT)
            then
          begin
  ...
  begin
    ...
    begin
      ...
      begin
        if not g_boSerieMagicing then
        begin
          if (g_NewStatus = sNone) and (not g_TargetCret.m_boDeath)
            and PlayScene.IsValidActor(g_TargetCret) then
          begin
            FillChar(keyvalue, SizeOf(TKeyBoardState), #0);
            ...
            begin
              ...
              if ((g_TargetCret.m_btRace <> RCC_USERHUMAN) and
                  ...
                  (g_TargetCret.m_btRace <> RCC_MERCHANT) and (Pos('(',
                    g_TargetCret.m_sUserName) = 0) //
                ) or (g_TargetCret.m_nNameColor = ENEMYCOLOR) // or ((ssShift in Shift) or (target.m_btRace <> RCC_MERCHANT and g_boNeedShift))
                or (((ssShift in Shift) or
                    ((g_TargetCret.m_btRace <> RCC_MERCHANT)
                      and g_Config.boNotNeedShift))
                { and (not PlayScene.EdChat.Visible) } ) then
              begin
  ...
  begin
    ...
    // 自动烈火
    if g_Config.boSmartFireHit and (GetTickCount - g_dwAutoFireTick > 1000 * 3)
      and (g_MySelf.m_Abil.MP >= 7) and (not g_boNextTimeFireHit) and
      (GetTickCount - g_dwLatestFireHitTick >= g_ServerConfig.nFireDelayTime)
      then
    begin
      g_dwAutoFireTick := GetTickCount;
      pm := FindMagic(26);
    ...
    // 自动开天
    if g_Config.boSmartKaitHit and (GetTickCount - g_dwAutoKTZTick > 1000 * 3)
      and (g_MySelf.m_Abil.MP >= 7) and (not g_boNextTimeKTZHit) and
      (GetTickCount - g_dwLatestKTZHitTick >= g_ServerConfig.nKTZDelayTime)
      then
    begin
      g_dwAutoKTZTick := GetTickCount;
      pm := FindMagic(58);
    ...
    // 自动破空剑
    if g_Config.boSmartPokHit and (GetTickCount - g_dwAutoPKJTick > 1000 * 3)
      and (g_MySelf.m_Abil.MP >= 7) and (not g_boNextTimePKJHit) and
      (GetTickCount - g_dwLatestPKJHitTick >= g_ServerConfig.nPKJDelayTime)
      then
    begin
      g_dwAutoPKJTick := GetTickCount;
      pm := FindMagic(43);
    ...
    // 自动逐日剑法
    if g_Config.boSmartSwordHit and
      (GetTickCount - g_dwAutoZRJFTick > 1000 * 3) and
      (g_MySelf.m_Abil.MP >= 7) and (not g_boNextTimeZRJFHit) and
      (GetTickCount - g_dwLatestZRJFHitTick >=
        g_ServerConfig.nZRJFDelayTime) then
    begin
      g_dwAutoZRJFTick := GetTickCount;
      pm := FindMagic(77);
    ...
    if g_Config.boSmartShield and ((g_MySelf.m_nState and $00100000) = 0) then
    begin // 智能盾
      pm := FindMagic(31);
      if pm <> nil then
      begin
        if not IsWarrSkill(pm.Def.wMagicId) then
        begin
          if (g_MySelf.m_btHorse > 0) and g_Config.boAutoHorse then
            SendOnHorse();
        end;
        if pm.Def.wSpell + pm.Def.btDefSpell <= g_MySelf.m_Abil.MP then
        begin
    ...
    // 动自练功
    if g_Config.boAutoUseMagic and (ActionKey = 0) then
    begin
      if (g_Config.nAutoUseMagicIdx >= 0) and
        (g_Config.nAutoUseMagicIdx < FrmDlg.DComboboxAutoUseMagic.Items.Count)
        then
      begin
        if GetTickCount - g_Config.dwAutoUseMagicTick >
          g_Config.nAutoUseMagicTime * 1000 then
        begin
          g_Config.dwAutoUseMagicTick := GetTickCount;
          PlayScene.ScreenXYfromMCXY(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, nX,
            nY);
          pm := PTClientMagic(FrmDlg.DComboboxAutoUseMagic.Items.Objects
              [g_Config.nAutoUseMagicIdx]);
          if not IsWarrSkill(pm.Def.wMagicId) then
          begin
            if (g_MySelf.m_btHorse > 0) and g_Config.boAutoHorse then
              SendOnHorse();
          ...
          UseMagic(nX, nY,
            PTClientMagic(FrmDlg.DComboboxAutoUseMagic.Items.Objects
                [g_Config.nAutoUseMagicIdx]));
        end;
      end;
    end;
    ...
    // 智能无极真气
    if g_Config.boSmartWjzq and
      (GetTickCount - g_Config.dwSmartWjzqTick >
        g_ServerConfig.nSkill50DelayTime) then
    begin
    ...
    case WaitingMsg.ident of
      SM_CHANGEMAP:
        begin
          ...
          PlayScene.SendMsg(SM_CHANGEMAP, 0, WaitingMsg.param
            { x } , WaitingMsg.tag { y } , WaitingMsg.series
            { darkness } , 0, 0, WaitingStr { mapname } );
      ...
      begin
        ...
        begin
          ...
          if CSocket.Socket.Connected then
            CSocket.Socket.SendText('$S' + g_sSelChrAddr + '/' + IntToStr
                (g_nSelChrPort) + '%');
...
begin
  ...
  begin
    ...
    if g_ConfigClient.btMainInterface in [1, 3] then
    begin
...
begin
  g_sSockText := '';
  g_sBufferText := '';
  if Assigned(g_PlugInfo.HookSocketConnect) then
  begin
    g_PlugInfo.HookSocketConnect();
  end;
  g_boServerConnected := True;
  if g_ConnectionStep = cnsStart then
...
begin
  g_boServerConnected := False;
  if (g_ConnectionStep in [cnsStart, cnsLogin]) and (not g_boSendLogin) then
  begin
    if not g_boShowVersionBmp then
    begin
...
begin
  ...
  if Assigned(g_PlugInfo.HookSocketRead) then
  begin
    if g_PlugInfo.HookSocketRead(PChar(Data), Length(Data)) then
      Exit;
  end;
...
begin
  if CSocket.Socket.Connected then
  begin
    if Assigned(g_PlugInfo.HookSendSocket) then
    begin
      if g_PlugInfo.HookSendSocket(PChar(sendstr), Length(sendstr)) then
        Exit;
    end;
...
procedure TfrmMain.SendFindUserItem(nWho: Integer; sData: string);
// 查询装备信息2008-3-5
var
  Msg: TDefaultMessage;
begin
...
begin
  ...
  SendSocket(EncodeMessage(Msg) + EncodeBuffer(@ue, SizeOf(TUserEntry))
...
begin
  ...
  SendSocket(EncodeMessage(Msg) + EncodeString(LoginID + '/' + IntToStr
        (Certification)));
end;
...
var
  sSendMsg: string;
begin
  sSendMsg := Format('**%s/%s/%d/%d/%d/%d', [LoginID, g_sSelChrName,
    Certification xor $3EB2C5CC, Certification xor $54B163FD,
...
begin
  if g_MySelf = nil then
    Exit;
  if GetTickCount - g_dwAutoShowMsgTick > 1000 * 60 then
  begin
    g_dwAutoShowMsgTick := GetTickCount;
    if g_sAutoShowMsg <> '' then
    begin
...
begin
  ...
  begin
    ...
    if Str = '/debug powerblock' then
    begin
      SendPowerBlock();
      Exit;
    end;
    if Str = '/debug screen' then
    begin
      g_boCheckBadMapMode := not g_boCheckBadMapMode;
      if g_boCheckBadMapMode then
        DScreen.AddSysMsg('On', 30, 40, clAqua)
      else
        DScreen.AddSysMsg('Off', 30, 40, clAqua);
      // 是否显示相关检查地图信息(用于调试)
      Exit;
    end;
    ...
    if Str[1] = '/' then
      Msg := MakeDefaultMsg(CM_SAY, 1, MakeWord(g_Config.btHearMsgFColor,
          g_btWhisperMsgFColor), 0, 1); // edited
    if (Str[1] <> '@') and (Str[1] <> '/') and (Str[1] <> '!') and
      (Str[1] <> '~') then
      Msg := MakeDefaultMsg(CM_SAY, 1, MakeWord(g_Config.btHearMsgFColor,
          g_btWhisperMsgFColor), 0, 1); // Edited
    ...
    if Str[1] = '/' then
    begin
      DScreen.AddChatBoardString(Str, GetRGB(g_btWhisperMsgFColor)
        { GetRGB(180) } , clWhite);
...
begin
  ...
  SendSocket(EncodeMessage(Msg) + EncodeBuffer(@UpgradeItemIndexs,
      SizeOf(TUpgradeItemIndexs)));
end;
...
procedure TfrmMain.SendGetSellItemGold(nItemServerIndex: Integer;
  sItemName: string);
var
  Msg: TDefaultMessage;
begin
  Msg := MakeDefaultMsg(CM_SENDGETSELLITEMGOLD, 0, LoWord(nItemServerIndex),
    HiWord(nItemServerIndex), 0);
  SendSocket(EncodeMessage(Msg) + EncodeString(sItemName));
end;

procedure TfrmMain.SendBuySellItem(nItemServerIndex: Integer;
  sItemName: string);
var
  Msg: TDefaultMessage;
begin
  Msg := MakeDefaultMsg(CM_SENDBUYSELLOFFITEM, 0, LoWord(nItemServerIndex),
    HiWord(nItemServerIndex), 0);
  SendSocket(EncodeMessage(Msg) + EncodeString(sItemName));
end;
...
procedure TfrmMain.SendTakeOnItemFromMasterBag(where: Byte; itmindex: Integer;
  itmname: string); // 从主人包裹穿装备
var
  Msg: TDefaultMessage;
begin
...
procedure TfrmMain.SendTakeOnItemFromHeroBag(where: Byte; itmindex: Integer;
  itmname: string); // 从英雄包裹穿装备
var
  Msg: TDefaultMessage;
begin
...
procedure TfrmMain.SendTakeOffItemToHeroBag(where: Byte; itmindex: Integer;
  itmname: string); // 装备脱下到英雄包裹
var
  Msg: TDefaultMessage;
begin
...
procedure TfrmMain.SendTakeOffItemToMasterBag(where: Byte; itmindex: Integer;
  itmname: string); // 装备脱下到主人包裹
var
  Msg: TDefaultMessage;
begin
...
procedure TfrmMain.SendHeroTakeOnItem(where: Byte; itmindex: Integer;
  itmname: string);
var
  Msg: TDefaultMessage;
begin
...
procedure TfrmMain.SendHeroTakeOffItem(where: Byte; itmindex: Integer;
  itmname: string);
var
  Msg: TDefaultMessage;
begin
...
procedure TfrmMain.SendRepairFirDragon(nType: Byte; itmindex: Integer;
  itmname: string);
var
  Msg: TDefaultMessage;
begin
...
procedure TfrmMain.SendSellOffItem(merchant, itemindex: Integer;
  itemname: string);
var
  Msg: TDefaultMessage;
begin
  Msg := MakeDefaultMsg(CM_SENDSELLOFFITEM, merchant, LoWord(itemindex),
    HiWord(itemindex), 0);
  SendSocket(EncodeMessage(Msg) + EncodeString(itemname));
end;
// 发送要修改的物品

procedure TfrmMain.SendChangeItem(merchant, itemindex: Integer;
  itemname: string);
var
  Msg: TDefaultMessage;
begin
  Msg := MakeDefaultMsg(CM_SENDCHANGEITEM, merchant, LoWord(itemindex),
    HiWord(itemindex), 0);
  SendSocket(EncodeMessage(Msg) + EncodeString(itemname));
end;
// 发送要修理的物品
...
var
  Msg: TDefaultMessage;
begin
  Msg := MakeDefaultMsg(CM_OPENSHOP, Integer(g_MySelf), nPage, nTabePage,
    nSuperTabePage);
  SendSocket(EncodeMessage(Msg));
end;
...
procedure TfrmMain.SendBuyStoreItem(sItems: string;
  nMakeIndex, nRecogId: Integer);
var
  Msg: TDefaultMessage;
begin
  Msg := MakeDefaultMsg(CM_BUYSTOREITEM, nRecogId, 0, 0, 0);
  SendSocket(EncodeMessage(Msg) + EncodeString(sItems + '/' + IntToStr
        (nMakeIndex)));
end;
...
begin
  ...
  for I := 0 to 14 do
  begin
    if g_StoreItems[I].Item.S.Name <> '' then
    begin
...
begin
  ...
  begin
    ...
    Msg := MakeDefaultMsg(CM_DOMAINNAME,
      StringCrc(EncryStrHex3(DecryptString(g_ConfigClient.sDomainName),
          IntToStr(nParam))), 0, LoWord(nParam), HiWord(nParam));
    SendSocket(EncodeMessage(Msg));
  end;
end;
{ --------------------------------------------------------------- }
...
begin
  if (g_MySelf.IsIdle) and (g_MySelf.m_nState and $04000000 = 0) and
...
begin
  ...
  LevelFastTime := _MIN(800,
    LevelFastTime + g_MySelf.m_nHitSpeed * g_nItemSpeed);
  ...
  if g_boAttackSlow then
    NextHitTime := dwHitTime { 1400 } - LevelFastTime + 1500
    // 腕力超过时，减慢攻击速度
  else
    NextHitTime := dwHitTime { 1400 } - LevelFastTime;

  if g_ServerConfig.boChgSpeed then
  begin
    // DebugOutStr('TfrmMain.CanNextHit1 AttackSpeed:'+IntToStr(g_MySelf.m_Abil.AttackSpeed)+' NextHitTime:'+IntToStr(NextHitTime));
    NextHitTime := NextHitTime - Trunc
      ((g_MySelf.m_Abil.AttackSpeed * NextHitTime) / 100);
...
begin
  ...
  try
    ...
    if g_sBufferText <> '' then
    begin
      mcnt := 0;
      while Length(g_sBufferText) >= 2 do
      begin
        if g_boMapMovingWait then
          Break;
        if Pos('!', g_sBufferText) <= 0 then
          Break;
  ...
  if (g_ConnectionStep = cnsPlay) and not g_SoftClosed then
  begin
    ...
    if g_boShowHearMsg then
      AutoSendSay(); // 自动喊话
{$I VMProtectBeginMutation.inc}
    if GetTickCount - g_dwOpenHomePageTick^ > 1000 * 60 * 60 * 1 then
    begin
      ...
      if RegInfo.boShare then
      begin
        if Assigned(g_PlugInfo.OpenHomePage) then
        begin
...
{$I VMProtectEnd.inc}
  ...
  begin
    ...
    while (GetTickCount - m_dwGetMsgTick < 200) and GetMessage(@ProcessMsg) do
    begin
  ...
  if g_boShowVersionBmp and (GetTickCount - g_dwShowVersionBmpTick >= 1000 * 3)
    then
  begin
    g_boShowVersionBmp := False;
    if g_ConnectionStep in [cnsStart, cnsLogin] then
    begin
      if CSocket.Socket.Connected then
      begin
        { if OneClickMode = toKornetWorld then begin
        ...
        g_ProcessesList.Clear;
        if g_ConfigClient.sCheckProcessesUrl <> '' then
        begin
          // HTTPGetCheckPro.URL := g_ConfigClient.sCheckProcessesUrl;
          // HTTPGetCheckPro.GetString;
        end;
...
begin
  ...
  if sData = 'UKTZ' then
    g_boNextTimeKTZHit := False; // 关闭开天斩
  if sData = 'UCID' then
    g_boNextTimePKJHit := False; // 关闭破空剑
  if sData = 'UZRJF' then
    g_boNextTimeZRJFHit := False; // 关闭破空剑
    ...
    0:
      begin
        FrmDlg.DMessageDlg('"' + MakeNewId +
            ...
            'Please use a different name.', [mbOk]);
        LoginScene.NewIdRetry(False);
      end;
    1:
      FrmDlg.DMessageDlg('Verfication Code error. Please re-enter the code!',
        [mbOk]);
    -2:
      FrmDlg.DMessageDlg('Account name has been banned', [mbOk]);
  else
    begin
      FrmDlg.DMessageDlg(
        'ID creation Faliled. Please check it does not contain space,\ special characters, or an illegible letter. Code: ' + IntToStr(nFailCode), [mbOk]);
    end;
  end;
  LoginScene.ShowLoginBox;
end;
...
begin
  case nFailCode of
    - 1:
      FrmDlg.DMessageDlg(
        'Wrong Password.\Please try again, if you have forgotten your password, \contact the support team.', [mbOk]);
    -2:
      FrmDlg.DMessageDlg(
        'Wrong Password 3 times in a row.\You will not be able to connect for a while.', [mbOk]);
    -3:
      FrmDlg.DMessageDlg(
        'This account is actually in use or locked by abnormal termination.\Please try again later.', [mbOk]);
    -4:
      FrmDlg.DMessageDlg(
        'This account has no access rights.\please change account,\or apply for paid registration.', [mbOk]);
    -5:
      FrmDlg.DMessageDlg(
        'Your account has expired or it may have been suspended', [mbOk]);
  else
    FrmDlg.DMessageDlg(
      'Account does not exist.\Please check your account name and try again.',
      [mbOk]);
  end;
...
begin
  case nFailCode of
    0:
      FrmDlg.DMessageDlg(
        '[Failure] Character name contains illegal letters. Code = 0', [mbOk]);
    2:
      FrmDlg.DMessageDlg('[Failure] This name already exists', [mbOk]);
    3:
      FrmDlg.DMessageDlg(
        '[Failure] You cannot make two characters on one account. \Please contact the Game Master.', [mbOk]);
    4:
      FrmDlg.DMessageDlg('[Failure] Character creating failure. Error=4',
        [mbOk]);
  else
    FrmDlg.DMessageDlg('[Failure] Unknown error. Please contact hompage.',
      [mbOk]);
  end;
end;
...
begin
  case nFailCode of
    - 1:
      FrmDlg.DMessageDlg('Wrong password. Password cannot be changed.', [mbOk]);
    -2:
      FrmDlg.DMessageDlg
        ('Account is locked. Try again in a while. (5 minutes).', [mbOk]);
  else
    FrmDlg.DMessageDlg(
...
begin
  ...
  DScreen.AddChatBoardString(
    'Please download the correct Version and try again.', clyellow, clRed);
  CSocket.Close;
end;
...
begin
  ...
  // g_sMapName := sText;
  PlayScene.SendMsg(DefMsg.ident, 0, DefMsg.param { x } , DefMsg.tag
    { y } , DefMsg.series { darkness } , 0, 0, sText { mapname } );
end;

procedure TfrmMain.ClientGetUserLogin(DefMsg: pTDefaultMessage; sData: string);
var
...
{$I VMProtectBeginMutation.inc}
  ...
  PlayScene.SendMsg(DefMsg.ident, DefMsg.Recog, DefMsg.param { x } , DefMsg.tag
    { y } , DefMsg.series { dir } , MsgWL.lParam1
    { desc.Feature } , MsgWL.lParam2 { desc.Status } , '');
...
{$I VMProtectEnd.inc}
  if LoByte(LoWord(MsgWL.lTag1)) = 1 then
    g_boAllowGroup := True
  else
    g_boAllowGroup := False;
  g_boServerChanging := False;
...
procedure TfrmMain.ClientGetObjTurn(DefMsg: pTDefaultMessage; sData: string);
var
  sBody2: string;
  sBody: string;
  sColor: string;
...
begin
  ...
  begin
    ...
    PlayScene.SendMsg(DefMsg.ident, DefMsg.Recog, DefMsg.param
      { x } , DefMsg.tag { y } , DefMsg.series
      { dir + light } , CharDesc.feature, CharDesc.Status, '');
    if sBody <> '' then
    begin
      ...
      begin
        ...
        if (Actor <> g_MySelf) and (Actor <> g_MyHero) then
        begin
...
procedure TfrmMain.ClientGetBackStep(DefMsg: pTDefaultMessage; sData: string);
var
  sBody2: string;
  sBody: string;
  sColor: string;
...
begin
  ...
  begin
    PlayScene.SendMsg(DefMsg.ident, DefMsg.Recog, DefMsg.param
      { x } , DefMsg.tag { y } , 0, 0, 0, '')
  end;
end;

procedure TfrmMain.ClientSpaceMoveShow(DefMsg: pTDefaultMessage; sData: string);
var
  sBody2: string;
  sBody: string;
  sColor: string;
...
begin
  ...
  begin
    ...
    if DefMsg.Recog <> g_MySelf.m_nRecogId then
    begin
      PlayScene.NewActor(DefMsg.Recog, DefMsg.param, DefMsg.tag, DefMsg.series,
...
procedure TfrmMain.ClientObjWalk(DefMsg: pTDefaultMessage; sData: string);
var
  CharDesc: TCharDesc;
  Actor: TActor;
begin
  if Length(sData) >= g_nCodeMsgSize then
  begin
    DecodeBuffer(sData, @CharDesc, SizeOf(TCharDesc));
    if (DefMsg.Recog <> g_MySelf.m_nRecogId) or (DefMsg.ident = SM_RUSH) or
...
procedure TfrmMain.ClientObjRun(DefMsg: pTDefaultMessage; sData: string);
var
  CharDesc: TCharDesc;
  Actor: TActor;
begin
...
procedure TfrmMain.ClientObjMoveFail(DefMsg: pTDefaultMessage; sData: string);
var
  CharDesc: TCharDesc;
begin
  if Length(sData) >= g_nCodeMsgSize then
  begin
    ...
    DecodeBuffer(sData, @CharDesc, SizeOf(TCharDesc));
    if DefMsg.Recog <> g_MySelf.m_nRecogId then
      PlayScene.SendMsg(SM_TURN, DefMsg.Recog, DefMsg.param { x } , DefMsg.tag
        { y } , DefMsg.series
        { dir+light } , CharDesc.feature, CharDesc.Status, '');
  end;
end;

procedure TfrmMain.ClientObjButch(DefMsg: pTDefaultMessage; sData: string);
var
  // CharDesc: TCharDesc;
  Actor: TActor;
begin
  ...
  begin
    Actor := PlayScene.FindActor(DefMsg.Recog);
    Actor.SendMsg(SM_SITDOWN, DefMsg.param { x } , DefMsg.tag
      { y } , DefMsg.series { dir } , 0, 0, '', 0);
  end;
end;

procedure TfrmMain.ClientObjHit(DefMsg: pTDefaultMessage; sData: string);
var
  Actor: TActor;
begin
  // DScreen.AddChatBoardString('DefMsg.ident: ' + IntToStr(DefMsg.ident), clWhite, clPurple);
  if (DefMsg.Recog <> g_MySelf.m_nRecogId) or (DefMsg.ident = SM_101HIT) then
  begin
    Actor := PlayScene.FindActor(DefMsg.Recog);
    if Actor <> nil then
    begin
      Actor.SendMsg(DefMsg.ident, DefMsg.param { x } , DefMsg.tag
        { y } , DefMsg.series { dir } , 0, 0, '', 0);
      if DefMsg.ident = SM_HEAVYHIT then
      begin
        if sData <> '' then
  ...
  begin // 合击
    if DefMsg.ident in [SM_60HIT .. SM_62HIT] then
    begin
      // DScreen.AddChatBoardString('DefMsg.ident:'+IntToStr(DefMsg.ident), clGreen, clWhite);
      g_MySelf.SendMsg(DefMsg.ident, DefMsg.param { x } , DefMsg.tag
        { y } , DefMsg.series { dir } , 0, 0, '', 0);
    end;
  end;
end;

procedure TfrmMain.ClientObjFlyAxe(DefMsg: pTDefaultMessage; sData: string);
var
...
procedure TfrmMain.ClientObjLighting(DefMsg: pTDefaultMessage; sData: string);
var
...
procedure TfrmMain.ClientObjSpell(DefMsg: pTDefaultMessage; sData: string);
begin
  UseMagicSpell(DefMsg.Recog { who } , DefMsg.series
    { effectnum } , DefMsg.param { tx } , DefMsg.tag
    { y } , Str_ToInt(sData, 0));
end;

procedure TfrmMain.ClientObjMagicFire(DefMsg: pTDefaultMessage; sData: string);
var
  nTarget: Integer;
begin
  DecodeBuffer(sData, @nTarget, SizeOf(Integer));
  UseMagicFire(DefMsg.Recog { who } , LoByte(DefMsg.series)
    { efftype } , HiByte(DefMsg.series) { effnum } , DefMsg.param
    { tx } , DefMsg.tag { y } , nTarget);
end;
...
begin
  ...
  g_boDoFadeOut := False;
  FrmDlg.DMessageDlg(
    'Server connection has been terminated.\Connection time limit has been reached'
      , [mbOk]);
  Close;
end;

procedure TfrmMain.ClientObjDeath(DefMsg: pTDefaultMessage; sData: string);
var
  Actor: TActor;
  CharDesc: TCharDesc;
begin
  ...
  begin
    ...
    begin
      Actor.SendMsg(DefMsg.ident, DefMsg.param { x } , DefMsg.tag
        { y } , DefMsg.series
        { damage } , CharDesc.feature, CharDesc.Status, '', 0);
      Actor.m_Abil.HP := 0;
      // Actor.m_Abil.Level := CharDesc.Level;
      // Actor.m_Abil.HP := CharDesc.HP;
      // Actor.m_Abil.MaxHP := CharDesc.MaxHP;
    ...
    begin
      PlayScene.SendMsg(SM_DEATH, DefMsg.Recog, DefMsg.param { x } , DefMsg.tag
        { y } , DefMsg.series
        { damage } , CharDesc.feature, CharDesc.Status, '');
    end;
  end;
end;

procedure TfrmMain.ClientObjSkeLeton(DefMsg: pTDefaultMessage; sData: string);
var
  CharDesc: TCharDesc;
begin
  if Length(sData) >= g_nCodeMsgSize then
  begin
    DecodeBuffer(sData, @CharDesc, SizeOf(TCharDesc));
    PlayScene.SendMsg(DefMsg.ident, DefMsg.Recog, DefMsg.param
      { HP } , DefMsg.tag { maxHP } , DefMsg.series
      { damage } , CharDesc.feature, CharDesc.Status, '');
  end;
end;

procedure TfrmMain.ClientObjAbility(DefMsg: pTDefaultMessage; sData: string);
begin
  if g_MySelf <> nil then
  begin
    ...
    if not g_boLoadUserConfig then
    begin
...
begin
  ...
  DScreen.AddSysMsg('Gained ' + IntToStr(LongWord(MakeLong(DefMsg.param,
          DefMsg.tag))) + ' Experience', SCREENWIDTH - 150, 150, clLime);
end;

procedure TfrmMain.ClientObjLevelUp(DefMsg: pTDefaultMessage);
begin
  g_MySelf.m_Abil.Level := MakeLong(DefMsg.param, DefMsg.tag);
  g_MySelf.SendMsg(SM_LEVELUP, g_MySelf.m_nRecogId, g_MySelf.m_nCurrX
    { X } , g_MySelf.m_nCurrY { Y } , g_MySelf.m_btDir { d } , 0, '', 0);
...
begin
  ...
  begin
    ...
    else if nDamage >= g_ServerConfig.nGreenNumber then
    begin
      Actor.AddHealthStatus(6, abs(nDamage));
    end
    else if nDamage > 0 then
    begin
    ...
    if Actor.m_Abil.MP - Health.MP < 0 then
    begin
      Actor.AddHealthStatus(3, abs(Actor.m_Abil.MP - Health.MP));
    end
    else if Actor.m_Abil.MP - Health.MP > 0 then
    begin
  ...
  begin
    if (g_MyHero <> nil) and (g_MyHero.m_nRecogId = DefMsg.Recog) then
    begin
...
procedure TfrmMain.ClientObjStruck(DefMsg: pTDefaultMessage; sData: string);
var
  Health: THealth;
  MessageBodyWL: TMessageBodyWL;
  Actor: TActor;
...
begin
  Actor := PlayScene.FindActor(DefMsg.Recog);
  if Assigned(g_PlugInfo.HookClientObjStruck) then
  begin
    if g_PlugInfo.HookClientObjStruck(@DefMsg, PChar(sData), Length(sData)) then
      Exit;
  end;
  if Actor <> nil then
  begin
    ...
    begin
      ...
      if g_Config.boStruckShield and (not g_MySelf.m_boDeath) and
        ((g_MySelf.m_nState and $00100000) = 0) then
      begin // 被攻击开盾
        ClientMagic := FindMagic(31);
        if ClientMagic <> nil then
        begin
          if ClientMagic.Def.wSpell + ClientMagic.Def.btDefSpell <=
            g_MySelf.m_Abil.MP then
          begin
    ...
    if Actor = g_MySelf then
    begin
      if not g_Config.boStable then
      begin
        Actor.UpdateMsg(SM_STRUCK, MessageBodyWL.lTag2, 0,
          MakeLong(DefMsg.tag, DefMsg.series)
          { damage } , MessageBodyWL.lParam1, MessageBodyWL.lParam2, '',
          MessageBodyWL.lTag1);
      end;
    end
    else
    ...
    nDamage := Actor.m_Abil.HP - Health.HP;
    if Actor.m_Abil.HP = Health.HP then
    begin
      Actor.AddHealthStatus(0, 0);
    end
    else if nDamage < 0 then
...
procedure TfrmMain.ClientObjChangeFace(DefMsg: pTDefaultMessage; sData: string);
var
  CharDesc: TCharDesc;
  Actor: TActor;
begin
...
procedure TfrmMain.ClientObjOpenHealth(DefMsg: pTDefaultMessage; sData: string);
var
  Actor: TActor;
  Health: THealth;
begin
...
begin
  Actor := PlayScene.FindActor(DefMsg.Recog);
  if (Actor <> nil) and (Actor <> g_MySelf) then
  begin
    DecodeBuffer(sData, @Health, SizeOf(THealth));
    Actor.m_Abil.HP := Health.HP;
    Actor.m_Abil.MaxHP := Health.MaxHP;
...
procedure TfrmMain.ClientGetMessage(DefMsg: pTDefaultMessage; sData: string);
var
  sMsg: string;
  Actor: TActor;
begin
  sMsg := DecodeString(sData);
  if Assigned(g_PlugInfo.HookClientGetMessage) then
  begin
    if g_PlugInfo.HookClientGetMessage(PChar(sMsg), Length(sMsg),
      GetRGB(LoByte(DefMsg.param)), GetRGB(HiByte(DefMsg.param))) then
      Exit;
  end;
  ...
  case DefMsg.ident of
    SM_GROUPMESSAGE:
      begin
        if g_boShowGroupMsg then
        begin
          DScreen.AddChatBoardString(sMsg, GetRGB(LoByte(DefMsg.param)),
            GetRGB(HiByte(DefMsg.param)));
        end;
      end;
    SM_GUILDMESSAGE:
      begin
        if g_boShowGuildMsg then
        begin
          sMsg := DScreen.AddChatBoardString(sMsg,
            GetRGB(LoByte(DefMsg.param)), GetRGB(HiByte(DefMsg.param)));
          FrmDlg.AddGuildChat(sMsg);
        end;
      end;
    SM_WHISPER:
      begin
        if g_boShowWhisperMsg then
        begin
          DScreen.AddChatBoardString(sMsg, GetRGB(LoByte(DefMsg.param)),
            GetRGB(HiByte(DefMsg.param)));
        end;
    ...
    SM_SYSMESSAGE:
      begin
        if not g_boShowSysMessage then
        begin
          DScreen.AddChatBoardString(sMsg, GetRGB(LoByte(DefMsg.param)),
            GetRGB(HiByte(DefMsg.param)));
        end
    ...
    SM_HEAR:
      begin
        if (LoByte(DefMsg.param) = 255) and (HiByte(DefMsg.param) = 255) then
        begin
    ...
    SM_MOVEMESSAGE:
      begin
        if (SCREENWIDTH = 1024) and (DefMsg.series = 330) then
        begin
          DScreen.AddMoveMsg(sMsg, LoByte(DefMsg.param), HiByte(DefMsg.param),
            DefMsg.tag, 495, DefMsg.Recog);
        end
        else
        begin
          DScreen.AddMoveMsg(sMsg, LoByte(DefMsg.param), HiByte(DefMsg.param),
            DefMsg.tag, DefMsg.series, DefMsg.Recog);
        end;
      end;
    SM_DELAYMESSAGE:
      begin
        DScreen.DrawDelayMsg.Add(DefMsg.Recog, sMsg, DefMsg.param,
          GetRGB(DefMsg.tag));
    ...
    SM_CENTERMESSAGE:
      begin
        // showmessage(sMsg);
        // DScreen.AddChatBoardString(sMsg, GetRGB(0), GetRGB(255));
        DScreen.DrawScreenCenterMsg.Add(sMsg, LoByte(DefMsg.param),
          HiByte(DefMsg.param), DefMsg.Recog);
      end;
  end;
end;

procedure TfrmMain.ClientGetUserName(DefMsg: pTDefaultMessage; sData: string);
var
  Actor: TActor;
  sUserName: string;
begin
...
begin
  if g_MySelf.m_nRecogId <> DefMsg.Recog then
  begin
    PlayScene.SendMsg(SM_HIDE, DefMsg.Recog, DefMsg.param { x } , DefMsg.tag
      { y } , 0, 0, 0, '');
  end;
end;

procedure TfrmMain.ClientObjDigUp(DefMsg: pTDefaultMessage; sData: string);
var
  MessageBodyWL: TMessageBodyWL;
  Actor: TActor;
begin
  ...
  if Actor = nil then
  begin
    Actor := PlayScene.NewActor(DefMsg.Recog, DefMsg.param, DefMsg.tag,
  ...
  begin
    ...
    Actor.SendMsg(DefMsg.ident, DefMsg.param { x } , DefMsg.tag
      { y } , DefMsg.series
      { dir + light } , MessageBodyWL.lParam1, MessageBodyWL.lParam2, '', 0);
  end;
end;
...
procedure TfrmMain.ClientGetShowEvent(DefMsg: pTDefaultMessage; sData: string);
var
  ShortMessage: TShortMessage;
  Event: TEvent;
begin
  DecodeBuffer(sData, @ShortMessage, SizeOf(TShortMessage));
  if DefMsg.param >= ET_SPACEDOOR_1 then
  begin
    Event := TSpaceDoorEvent.Create(DefMsg.Recog, LoWord(DefMsg.tag)
      { x } , DefMsg.series { y } , DefMsg.param - ET_SPACEDOOR_1 { e-type } );
  end
  else if DefMsg.param >= ET_FIREFLOWER_1 then
  begin
    Event := TFlowerEvent.Create(DefMsg.Recog, LoWord(DefMsg.tag)
      { x } , DefMsg.series { y } , DefMsg.param { e-type } );
  end
  else
  begin
    Event := TClEvent.Create(DefMsg.Recog, LoWord(DefMsg.tag)
      { x } , DefMsg.series { y } , DefMsg.param { e-type } );
  end;
  Event.m_nDir := 0;
...
begin
  ...
  PlayScene.SendMsg(SM_HEROLOGON, DefMsg.Recog, DefMsg.param { x } , DefMsg.tag
    { y } , DefMsg.series { dir } , wl.lParam1, // desc.Feature,
  ...
  if (wl.lTag2 = g_MySelf.m_nRecogId) then
  begin
...
begin // 英雄退出
  ...
  g_boHeroProtect := False;
  if (g_MyHero <> nil) { and (g_MyHero.m_nRecogId = nRecog) } then
  begin
    g_MyHero.m_dwDeleteTime := GetTickCount;
    UpDataFreeActorList(g_MyHero);
...
begin // 英雄登陆
  Actor := PlayScene.FindActor(DefMsg.Recog);
  if (Actor <> nil) { and (Actor = g_MyHero) } then
  begin
    Actor.SendMsg(SM_HEROLOGOUT, DefMsg.Recog, DefMsg.param, DefMsg.tag, 0, 0,
      '', 0);
  end;
end;
...
procedure TfrmMain.ClientObjHeroTakeOnOK(DefMsg: pTDefaultMessage;
  sData: string);
var
  I: Integer;
  sWhere: string;
...
begin
  ...
  begin
    ...
    begin
      ...
      if nWhere in [0 .. 12] then
      begin
        for I := Low(g_HeroItemArr) to High(g_HeroItemArr) do
        begin
          if g_HeroItemArr[I].S.Name <> '' then
          begin
            if (g_HeroItemArr[I].S.Name = sItemName) and
              (g_HeroItemArr[I].MakeIndex = nMakeIndex) then
            begin
...
procedure TfrmMain.ClientObjHeroTakeOffOK(DefMsg: pTDefaultMessage;
  sData: string);
var
  sWhere: string;
  nWhere: Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      if nWhere in [0 .. 12] then
      begin
        if (g_HeroUseItems[nWhere].S.Name = sItemName) and
          (g_HeroUseItems[nWhere].MakeIndex = nMakeIndex) then
        begin
...
begin
  ...
  begin
    ...
    DScreen.AddSysMsg('Hero Gained ' + IntToStr(LongWord(MakeLong(DefMsg.param,
            DefMsg.tag))) + ' Experience', SCREENWIDTH - 150, 150, clLime);
    // SCREENWIDTH - 100, 40, clGreen);
  end;
end;
...
begin
  ...
  begin
    ...
    g_MyHero.SendMsg(SM_LEVELUP, g_MyHero.m_nRecogId, g_MyHero.m_nCurrX
      { X } , g_MyHero.m_nCurrY { Y } , g_MyHero.m_btDir { d } , 0, '', 0);
    DScreen.AddSysMsg('Congratulations, Hero aquired a new level!', 30, 40,
      clAqua);
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if (g_ItemArr[I].S.Name = sItemName) and
            (g_ItemArr[I].MakeIndex = nMakeIndex) then
          begin
...
begin
  ...
  begin
    ...
    if nWhere in [0 .. 12] then
    begin
      if (g_UseItems[nWhere].S.Name = sItemName) and
        (g_UseItems[nWhere].MakeIndex = nMakeIndex) then
      begin
...
begin
  ...
  if DefMsg.Recog > g_MySelf.m_nGold then
  begin
    DScreen.AddSysMsg(IntToStr(DefMsg.Recog - g_MySelf.m_nGold)
        + ' ' + g_sGoldName + ' found.', 30, 40, clAqua);
  end;
  g_MySelf.m_nGold := DefMsg.Recog;
...
begin
  FrmDlg.LastestClickTime := GetTickCount;
  if DefMsg.ident <> SM_STORAGE_OK then
  begin
    if DefMsg.ident = SM_STORAGE_FULL then
    begin
      FrmDlg.DMessageDlg(
...
begin
  FrmDlg.LastestClickTime := GetTickCount;
  if DefMsg.ident <> SM_TAKEBACKSTORAGEITEM_OK then
  begin
    if DefMsg.ident = SM_TAKEBACKSTORAGEITEM_FULLBAG then
    begin
...
begin
  ...
  case nFailCode of
    1:
      FrmDlg.DMessageDlg('Item is sold out.', [mbOk]);
    2:
      FrmDlg.DMessageDlg('No more items can be carried.', [mbOk]);
    3:
      FrmDlg.DMessageDlg('You have not enough Gold to purchase item.', [mbOk]);
  end;
end;
...
begin
  ...
  case nFailCode of
    1:
      FrmDlg.DMessageDlg('An error occurred.', [mbOk]);
    2:
      FrmDlg.DMessageDlg('No more items can be carried.', [mbOk]);
    3:
      FrmDlg.DMessageDlg(g_sGoldName + 'required.', [mbOk]);
    4:
      FrmDlg.DMessageDlg('You lack necessary items.', [mbOk]);
  end;
end;
...
begin
  ...
  case nFailCode of
    - 1:
      FrmDlg.DMessageDlg('Already added in group.', [mbOk]);
    -2:
      FrmDlg.DMessageDlg('The name to be added in group is not correct.',
        [mbOk]);
    -3:
      FrmDlg.DMessageDlg(
        ...
        [mbOk]);
    -4:
      FrmDlg.DMessageDlg('The opponent is not on Allow Group.', [mbOk]);
  end;
end;
...
begin
  ...
  case nFailCode of
    - 1:
      FrmDlg.DMessageDlg('Group is not organized yet or you have no access.',
        [mbOk]);
    -2:
      FrmDlg.DMessageDlg('Already joined a Group.', [mbOk]);
    -3:
      FrmDlg.DMessageDlg('Members limit already reached.', [mbOk]);
    -4:
      FrmDlg.DMessageDlg('You are not in a Group yet.', [mbOk]);
    -5:
      FrmDlg.DMessageDlg(
        'You wanted to invite the human who joined grouping already to join other groups!', [mbOk]);
  end;
end;
...
begin
  g_dwQueryMsgTick := GetTickCount;
  FrmDlg.DMessageDlg(
    'Deal was cancelled .\To deal correctly you must face the other party.',
    [mbOk]);
end;
...
begin
  case nFailCode of
    1:
      FrmDlg.DMessageDlg('You must be Guild leader to add members.', [mbOk]);
    2:
      FrmDlg.DMessageDlg('Members who want to enter should face you.', [mbOk]);
    3:
      FrmDlg.DMessageDlg('This person already joined your Guild.', [mbOk]);
    4:
      FrmDlg.DMessageDlg('This person is already in a Guild.', [mbOk]);
    5:
      FrmDlg.DMessageDlg('Opponent is not allowed to enter the Guild.', [mbOk]);
  end;
end;
...
begin
  case nFailCode of
    1:
      FrmDlg.DMessageDlg('Only Guild leaders can remove members.', [mbOk]);
    2:
      FrmDlg.DMessageDlg('This person is not a member', [mbOk]);
    3:
      FrmDlg.DMessageDlg('Guild chief cannot withdraw himself.', [mbOk]);
    4:
      FrmDlg.DMessageDlg('This person is not a member', [mbOk]);
  end;
end;
...
begin
  case nFailCode of
    - 2:
      FrmDlg.DMessageDlg('[ChangeError] Chief place is empty.', [mbOk]);
    -3:
      FrmDlg.DMessageDlg('[ChangeError] Position of chief is empty.', [mbOk]);
    -4:
      FrmDlg.DMessageDlg(
        '[ChangeError] Guild chief cannot be more than 2 people.', [mbOk]);
    -5:
      FrmDlg.DMessageDlg('[ChangeError] New Guild chief has to be connected.',
        [mbOk]);
    -6:
      FrmDlg.DMessageDlg('[ChangeError] Cannot AddMember/DelMember.', [mbOk]);
    -7:
      FrmDlg.DMessageDlg('[ChangeError] Between 0 and 99', [mbOk]);
  end;
end;
...
begin
  case nFailCode of
    - 1:
      FrmDlg.DMessageDlg('You need to face the other guild leader.', [mbOk]);
    -2:
      FrmDlg.DMessageDlg('Failure in alliance.', [mbOk]);
    -3:
      FrmDlg.DMessageDlg(
        'You should face with Guild chief with whom you want to ally.', [mbOk]);
    -4:
      FrmDlg.DMessageDlg('Other Guild chief do not allow alliance.', [mbOk]);
  end;
end;
...
begin
  case nFailCode of
    - 1:
      FrmDlg.DMessageDlg('You have no access.', [mbOk]);
    -2:
      FrmDlg.DMessageDlg('You are not in alliance with that Guild.', [mbOk]);
    -3:
      FrmDlg.DMessageDlg('This is not an existing Guild.', [mbOk]);
  end;
end;
...
begin
  ...
  case nFailCode of
    - 1:
      FrmDlg.DMessageDlg('already joined Guild.', [mbOk]);
    -2:
      FrmDlg.DMessageDlg('You don''t have the required registration fee.',
        [mbOk]);
    -3:
      FrmDlg.DMessageDlg('You don''t have the required item of registration.',
        [mbOk]);
  else
    FrmDlg.DMessageDlg('Name must between 1 to 16 characters', [mbOk]);
  end;
end;
...
procedure TfrmMain.ClientObjPalyDice(DefMsg: pTDefaultMessage; sData: string);
var
  MessageBodyWL: TMessageBodyWL;
  sBody: string;
begin
  sBody := Copy(sData, GetCodeMsgSize(SizeOf(TMessageBodyWL) * 4 / 3) + 1,
    Length(sData));
  DecodeBuffer(sData, @MessageBodyWL, SizeOf(TMessageBodyWL));
  sBody := DecodeString(sBody);
  FrmDlg.m_nDiceCount := DefMsg.param; // QuestActionInfo.nParam1
  FrmDlg.m_Dice[0].nDicePoint := LoByte(LoWord(MessageBodyWL.lParam1));
  // UserHuman.m_DyVal[0]
  FrmDlg.m_Dice[1].nDicePoint := HiByte(LoWord(MessageBodyWL.lParam1));
  // UserHuman.m_DyVal[0]
  FrmDlg.m_Dice[2].nDicePoint := LoByte(HiWord(MessageBodyWL.lParam1));
  // UserHuman.m_DyVal[0]
  FrmDlg.m_Dice[3].nDicePoint := HiByte(HiWord(MessageBodyWL.lParam1));
  // UserHuman.m_DyVal[0]

  FrmDlg.m_Dice[4].nDicePoint := LoByte(LoWord(MessageBodyWL.lParam2));
  // UserHuman.m_DyVal[0]
  FrmDlg.m_Dice[5].nDicePoint := HiByte(LoWord(MessageBodyWL.lParam2));
  // UserHuman.m_DyVal[0]
  FrmDlg.m_Dice[6].nDicePoint := LoByte(HiWord(MessageBodyWL.lParam2));
  // UserHuman.m_DyVal[0]
  FrmDlg.m_Dice[7].nDicePoint := HiByte(HiWord(MessageBodyWL.lParam2));
  // UserHuman.m_DyVal[0]

  FrmDlg.m_Dice[8].nDicePoint := LoByte(LoWord(MessageBodyWL.lTag1));
  // UserHuman.m_DyVal[0]
  FrmDlg.m_Dice[9].nDicePoint := HiByte(LoWord(MessageBodyWL.lTag1));
  // UserHuman.m_DyVal[0]
  FrmDlg.DialogSize := 0;
  FrmDlg.DMessageDlg('', []);
...
begin
  case nFailCode of
    - 1:
      FrmDlg.DMessageDlg('[Error]Not enough ' + g_sGoldName, [mbOk]);
    -2:
      FrmDlg.DMessageDlg('[Error]Your bag is full', [mbOk]);
    -3:
      FrmDlg.DMessageDlg('[Error]Item does not exist', [mbOk]);
  else
    FrmDlg.DMessageDlg('[Error]Unknown error', [mbOk]);
  end;
end;
...
begin
  case nFailCode of
    1:
      FrmDlg.DMessageDlg('[Error]Your Item does not exist', [mbOk]);
    2:
      FrmDlg.DMessageDlg('[Error]Your bag is full', [mbOk]);
    3:
      FrmDlg.DMessageDlg('[Error]Not enough ' + g_sGoldName, [mbOk]);
...
begin
  ...
  case nFailCode of
    - 1, -4:
      FrmDlg.DMessageDlg('This item cannot be consigned', [mbOk]);
    -3:
      FrmDlg.DMessageDlg(
        'You have reached the maximum number of Consigned items', [mbOk]);
  else
    FrmDlg.DMessageDlg('Unknown error', [mbOk]);
  end;
end;
...
begin
  ...
  DScreen.AddChatBoardString('You have successfully upgraded the Item.',
    clWhite, clRed);
  // DebugOutStr('TfrmMain.ClientObjUpgradeItemOK EncodeBuffer:' + EncodeBuffer(@g_UpgradeItems[0], SizeOf(TClientItem)));
  // FrmDlg.DMessageDlg('TfrmMain.ClientObjChangeItemOK！！！', [mbOk]);
end;

procedure TfrmMain.ClientObjUpgradeItemFail(nFailCode: Integer; sData: string);
var
  Item: TClientItem;
begin
  ...
  case nFailCode of
    0:
      begin
        ...
        DScreen.AddChatBoardString('You have failed to upgrade this Item.',
          clWhite, clRed);
      end;
    -1:
      begin
        ...
        DScreen.AddChatBoardString('You have broken this Item.', clWhite,
          clRed);
      end;
    -2:
      begin
...
procedure TfrmMain.ClientObjOpenBoxOK(DefMsg: pTDefaultMessage; sData: string);
var
  I, Index: Integer;
  Str, Data, sMsg: string;
  cu: TClientItem;
begin
  if DefMsg.param > 0 then
  begin
...
begin
  if g_OpenBoxingItem.S.Name <> '' then
  begin
...
procedure TfrmMain.ClientGetDelChr(DefMsg: pTDefaultMessage; sData: string);
var
  I: Integer;
  Data, sMsg: string;
  DelChar: TDelChar;
begin
  ...
  begin
    DelChrCount := 0;
    SelDelChar := -1;
    for I := Low(DelCharArray) to High(DelCharArray) do
    begin
    ...
    begin
      ...
      begin
        ...
        for I := Low(DelCharArray) to High(DelCharArray) do
        begin
          if DelCharArray[I].sChrName = '' then
          begin
...
procedure TfrmMain.ClientGetRandomDIB(boOpen: Boolean; BitCount: Integer;
  sData: string);
var
  I, nSize, nX: Integer;
  OColor: TColor;
...
begin
  ...
  begin
    ...
    g_RandomSurface.TextOut(15 + Random(10), 0, DecryptString(sData));
    // $00050505
...
begin
  ...
  case nFailCode of
    0:
      FrmDlg.DMessageDlg('You must face an opponent to challenge them', [mbOk]);
    -1:
      FrmDlg.DMessageDlg('Your challenge has yet to be accepted', [mbOk]);
    -2:
      FrmDlg.DMessageDlg('Challenge request has exceeded limit', [mbOk]);
  else
    FrmDlg.DMessageDlg('Challenge request failed', [mbOk]);
  end;
end;
...
begin
  MoveDuelItemToBag;
  if g_DuelDlgItem.S.Name <> '' then
  begin
    AddItemBag(g_DuelDlgItem);
    g_DuelDlgItem.S.Name := '';
  end;
  if g_nDuelGold > 0 then
  begin
...
begin
  ...
  begin
    ...
    if (g_nStoreMasterRecogId = DefMsg.Recog) and (not g_MySelf.m_boStartStore)
      then
    begin
  ...
  begin
    ...
    begin
      ...
      if (g_nStoreMasterRecogId = DefMsg.Recog) and (not Actor.m_boStartStore)
        then
      begin
        FrmDlg.CloseUserStoreDlg;
      end
...
procedure TfrmMain.ClientObjBuyStoreItemFail(DefMsg: pTDefaultMessage);
// 摆摊物品购买失败
begin
  case DefMsg.Recog of
    - 1:
      FrmDlg.DMessageDlg('[Error] You lack' + GetStorePriceName(DefMsg.tag),
        [mbOk]);
    -2:
      FrmDlg.DMessageDlg('Item does not exist', [mbOk]);
    -3:
      FrmDlg.DMessageDlg('Bag is ful', [mbOk]);
  else
    FrmDlg.DMessageDlg('Unknown Error', [mbOk]);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      LegendMap.Stop;
      DScreen.AddChatBoardString(Format(
          'You have reached the Coordinates set (%d:%d).', [nX, nY]),
        GetRGB(154), clWhite);
    end
    else
    begin
      ...
      LegendMap.EndY := nY;
      DScreen.AddChatBoardString(Format(
          'Automatically traveling to the location at (%d:%d), press any mouse button to stop.', [nX, nY]), GetRGB(154), clWhite);
    end;
  end;
end;

procedure TfrmMain.ClientTakeOnItem(nMakeIndex, nWhere: Integer;
  sItemName: string);
var
  where: Integer;
  I: Integer;
...
begin
  ...
  begin
    ...
    begin
      if g_ItemArr[I].S.Name <> '' then
      begin
        if (CompareText(g_ItemArr[I].S.Name, sItemName) = 0) and
          (g_ItemArr[I].MakeIndex = nMakeIndex) then
        begin
          Item := g_ItemArr[I];
          g_ItemArr[I].S.Name := '';
    ...
    begin
      SendDelayMsg(g_MySelf, SM_TAKEONITEM, nWhere, nMakeIndex, 0, 0,
        sItemName, 1000);
...
procedure TfrmMain.ClientTakeOffItem(nMakeIndex, nWhere: Integer;
  sItemName: string);
begin
  if g_MySelf = nil then
    Exit;
  if nWhere in [0 .. 12] then
  begin
    if (g_UseItems[nWhere].S.Name <> '') and
      (g_UseItems[nWhere].S.Name = sItemName) and
      (g_UseItems[nWhere].MakeIndex = nMakeIndex) then
    begin
      g_UseItems[nWhere].S.Name := '';
    end
    else
      SendDelayMsg(g_MySelf, SM_TAKEOFFITEM, nWhere, nMakeIndex, 0, 0,
        sItemName, 1000);
  end;
end;

procedure TfrmMain.ClientHeroTakeOnItem(nMakeIndex, nWhere: Integer;
  sItemName: string);
var
  where: Integer;
  I: Integer;
...
begin
  ...
  begin
    ...
    begin
      if g_HeroItemArr[I].S.Name <> '' then
      begin
        if (CompareText(g_HeroItemArr[I].S.Name, sItemName) = 0) and
          (g_HeroItemArr[I].MakeIndex = nMakeIndex) then
        begin
    ...
    begin
      SendDelayMsg(g_MyHero, SM_HEROTAKEONITEM, nWhere, nMakeIndex, 0, 0,
        sItemName, 1000);
...
procedure TfrmMain.ClientHeroTakeOffItem(nMakeIndex, nWhere: Integer;
  sItemName: string);
begin
  if g_MyHero = nil then
    Exit;
  if nWhere in [0 .. 12] then
  begin
    if (g_HeroUseItems[nWhere].S.Name <> '') and
      (g_HeroUseItems[nWhere].S.Name = sItemName) and
      (g_HeroUseItems[nWhere].MakeIndex = nMakeIndex) then
    begin
      g_HeroUseItems[nWhere].S.Name := '';
    end
    else
      SendDelayMsg(g_MyHero, SM_HEROTAKEOFFITEM, nWhere, nMakeIndex, 0, 0,
        sItemName, 1000);
  end;
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          for II := 0 to Actor.m_HealthList.Count - 1 do
          begin
...
begin
  ...
  if g_MySelf.m_btDir <> btDir then
    g_MySelf.SendMsg(CM_TURN, g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, btDir, 0,
      0, '', 0);
...
begin
  ...
  if (g_nStoreMasterRecogId = g_MySelf.m_nRecogId)
    and FrmDlg.DUserStore.Visible then
  begin
    FrmDlg.CloseUserStoreDlg;
  end;
  g_sStoreMasterName := '';
...
begin
  ...
  if Actor <> nil then
  begin
    Actor.SendMsg(DefMsg.ident, DefMsg.Recog, DefMsg.param { x } , DefMsg.tag
      { y } , Actor.m_btDir, 0, '', 0);
    // g_MySelf.SendMsg(SM_LEVELUP, g_MySelf.m_nRecogId, g_MySelf.m_nCurrX {X}, g_MySelf.m_nCurrY {Y}, g_MySelf.m_btDir {d}, 0, '', 0);
  end;
end;

procedure TfrmMain.ClientObjCartInfo(DefMsg: pTDefaultMessage;
  sCharName: string); // 镖车信息
var
  I: Integer;
  CartInfo: pTCartInfo;
  boFind: Boolean;
begin
  ...
  try
    boFind := False;
    for I := 0 to g_CartInfoList.Count - 1 do
    begin
      CartInfo := g_CartInfoList.Items[I];
      if CartInfo.nRecogId = DefMsg.Recog then
      begin
...
begin
  ...
  begin
    ...
    if g_ServerConfig.boCheckSpeedHack and (sData <> '') then
    begin
  ...
  nCode := 1;
  if Length(sDataBlock) < DEFBLOCKSIZE then
  begin
    if sDataBlock[1] = '=' then
    begin
      sData := Copy(sDataBlock, 2, Length(sDataBlock) - 1);
  ...
  sBody := Copy(sDataBlock, DEFBLOCKSIZE + 1,
    Length(sDataBlock) - DEFBLOCKSIZE);
  DefMsg := DecodeMessage(sDefMsg);
  if Assigned(g_PlugInfo.HookDecodeMessagePacket) then
  begin
    if g_PlugInfo.HookDecodeMessagePacket(@DefMsg, PChar(sBody), @Buffer, nLen)
      then
      Exit;
    if nLen > 0 then
    begin
  ...
  begin
    case DefMsg.ident of
      SM_GETREGINFO:
        ClientGetRegInfo(@DefMsg, sBody);
      SM_SERVERCONFIG:
        ClientGetServerConfig(@DefMsg, sBody);
      SM_RANDOMCODE:
        ClientGetRandomCode(sBody, DefMsg.Recog);
      SM_NEWID_SUCCESS:
        ClientNewIDSuccess();
      SM_NEWID_FAIL:
      ...
      SM_NEEDUPDATE_ACCOUNT:
        ClientGetNeedUpdateAccount(sBody);
      SM_UPDATEID_SUCCESS:
        ClientUpdateAccountSuccess();
      SM_UPDATEID_FAIL:
        ClientUpdateAccountFail();
      SM_PASSOK_SELECTSERVER:
        ClientGetPasswordOK(@DefMsg, sBody);
      SM_SERVERNAME:
        ClientGetServerName(@DefMsg, sBody); // 获取服务器列表
      SM_SELECTSERVER_OK:
      ...
      SM_CHGPASSWD_SUCCESS:
        ClientChgPasswdSuccess();
      SM_CHGPASSWD_FAIL:
        ClientChgPasswdFail(DefMsg.Recog);
      SM_DELCHR_SUCCESS:
        SendQueryChr();
      SM_DELCHR_FAIL:
        ClientDelChrFail(DefMsg.Recog);
      SM_STARTPLAY:
      ...
      SM_FINDDELCHR:
        ClientGetDelChr(@DefMsg, sBody);
      SM_FINDDELCHR_SUCCESS:
        ClientGetDelChrOK(sBody);
      SM_FINDDELCHR_FAIL:
        ClientGetDelChrFail(DefMsg.Recog);
      SM_SENDRANDOMCODE:
        ClientGetRandomDIB(DefMsg.Recog = 0, DefMsg.param, sBody);
      // 获取验证码图片
      SM_OUTOFCONNECTION:
        ClientOutofConnection();
      SM_NEWMAP, SM_LOGON, SM_RECONNECT, SM_SENDNOTICE:
        ;
    else
      Exit;
    end;
  ...
  begin
    if DefMsg.ident = SM_CHANGEMAP then
    begin
      WaitingMsg := DefMsg;
  ...
  case DefMsg.ident of
    ...
    SM_SERVERCONFIG:
      ClientGetServerConfig(@DefMsg, sBody);
    SM_RECONNECT:
      ClientGetReconnect(sBody);
    SM_TIMECHECK_MSG:
      ;
    SM_AREASTATE:
      ClientGetAreaState(DefMsg.Recog);
    SM_MAPDESCRIPTION:
      ClientGetMapDescription(@DefMsg, sBody);
    SM_GAMEGOLDNAME:
      ClientGetGameGoldName(@DefMsg, sBody);
    SM_ADJUST_BONUS:
      ClientGetAdjustBonus(DefMsg.Recog, sBody);
    SM_MYSTATUS:
      ClientGetMyStatus(@DefMsg);
    SM_TURN:
      ClientGetObjTurn(@DefMsg, sBody);
    SM_BACKSTEP:
      ClientGetBackStep(@DefMsg, sBody);
    SM_SPACEMOVE_HIDE, SM_SPACEMOVE_HIDE2:
      ClientSpaceMoveHide(@DefMsg);
    SM_SPACEMOVE_SHOW, SM_SPACEMOVE_SHOW2:
      ClientSpaceMoveShow(@DefMsg, sBody);
    SM_WALK, SM_RUSH, SM_RUSHKUNG:
      ClientObjWalk(@DefMsg, sBody);
    SM_RUN, SM_HORSERUN:
      ClientObjRun(@DefMsg, sBody);
    SM_CHANGELIGHT:
      ClientChangeLigth(@DefMsg);
    SM_LAMPCHANGEDURA:
      ClientLampChangeDura(@DefMsg);
    SM_MOVEFAIL:
      ClientObjMoveFail(@DefMsg, sBody);
    SM_BUTCH, SM_SITDOWN:
      ClientObjButch(@DefMsg, sBody);
    SM_HIT, SM_HEAVYHIT, SM_POWERHIT, SM_LONGHIT, SM_WIDEHIT, SM_BIGHIT,
      SM_FIREHIT, SM_CRSHIT, SM_TWINHIT, SM_KTHIT, SM_PKHIT, SM_60HIT,
      SM_61HIT, SM_62HIT, SM_SUPERFIREHIT,

      SM_ZRJFHIT, SM_100HIT, SM_101HIT, SM_102HIT, SM_103HIT:
      ClientObjHit(@DefMsg, sBody);

    SM_FLYAXE:
      ClientObjFlyAxe(@DefMsg, sBody);
    SM_LIGHTING:
      ClientObjLighting(@DefMsg, sBody);
    SM_SPELL:
      ClientObjSpell(@DefMsg, sBody);
    SM_MAGICFIRE:
      ClientObjMagicFire(@DefMsg, sBody);
    SM_MAGICFIRE_FAIL:
      ClientObjMagicFireFail(@DefMsg);

    SM_OUTOFCONNECTION:
      ClientOutofConnection();
    SM_DEATH, SM_NOWDEATH:
      ClientObjDeath(@DefMsg, sBody);
    SM_SKELETON, SM_ALIVE:
      ClientObjSkeLeton(@DefMsg, sBody);
    SM_ABILITY:
      ClientObjAbility(@DefMsg, sBody);
    SM_SUBABILITY:
      ClientObjSubAbility(@DefMsg);
    SM_DAYCHANGING:
      ClientDayChanging(@DefMsg);
    SM_WINEXP:
      ClientObjWinExp(@DefMsg);
    SM_LEVELUP:
      ClientObjLevelUp(@DefMsg);
    SM_HEALTHSPELLCHANGED:
      ClientObjHealthSpellChanged(@DefMsg, sBody);
    SM_STRUCK:
      ClientObjStruck(@DefMsg, sBody);
    SM_CHANGEFACE:
      ClientObjChangeFace(@DefMsg, sBody);
    SM_PASSWORD:
      ClientObjInputPassword();
    SM_OPENHEALTH:
      ClientObjOpenHealth(@DefMsg, sBody);
    SM_CLOSEHEALTH:
      ClientObjCloseHealth(@DefMsg);
    SM_INSTANCEHEALGUAGE:
      ClientObjInstanceOpenHealth(@DefMsg, sBody);
    SM_BREAKWEAPON:
      ClientObjBreakWeapon(@DefMsg);
    SM_GROUPMESSAGE, SM_GUILDMESSAGE, SM_WHISPER, SM_CRY, SM_SYSMESSAGE,
      SM_HEAR, SM_MOVEMESSAGE, SM_DELAYMESSAGE, SM_DELETEDELAYMESSAGE,
      SM_CENTERMESSAGE:
      ClientGetMessage(@DefMsg, sBody);

    SM_USERNAME:
      ClientGetUserName(@DefMsg, sBody);
    SM_CHANGENAMECOLOR:
      ClientObjChangeNameColor(@DefMsg);
    SM_HIDE, SM_GHOST, SM_DISAPPEAR:
      ClientObjHide(@DefMsg);
    SM_DIGUP:
      ClientObjDigUp(@DefMsg, sBody);
    SM_DIGDOWN:
      ClientObjDigDown(@DefMsg);
    SM_SHOWEVENT:
      ClientGetShowEvent(@DefMsg, sBody);
    SM_HIDEEVENT:
      ClientGetHideEvent(@DefMsg);
    SM_ADDITEM:
      ClientGetAddItem(sBody);
    SM_BAGITEMS:
      ClientGetBagItmes(@DefMsg, sBody);
    SM_UPDATEITEM:
      ClientGetUpdateItem(sBody);
    SM_UPDATEITEM2:
      ClientGetUpdateItem2(sBody);
    SM_DELITEM:
      ClientGetDelItem(sBody);
    SM_DELITEMS:
      ClientGetDelItems(sBody);
    SM_DROPITEM_SUCCESS:
      DelDropItem(DecodeString(sBody), DefMsg.Recog);
    SM_DROPITEM_FAIL:
      ClientGetDropItemFail(DecodeString(sBody), DefMsg.Recog);
    SM_ITEMSHOW:
      ClientGetShowItem(@DefMsg, sBody);
    SM_ITEMHIDE:
      ClientGetHideItem(@DefMsg);
    SM_OPENDOOR_OK:
      Map.OpenDoor(DefMsg.param, DefMsg.tag);
    SM_OPENDOOR_LOCK:
      DScreen.AddSysMsg('此门被锁定！', 30, 40, clAqua);
    SM_CLOSEDOOR:
      Map.CloseDoor(DefMsg.param, DefMsg.tag);

    { ==================================排行榜================================= }
    SM_SENGRANKING:
      ClientGetRanking(@DefMsg, sBody);
    SM_SENGMYRANKING_FAIL:
      FrmDlg.DMessageDlg('Unable to get list', [mbOk]);
    { ==================================英雄相关================================= }
    SM_HEROLOGON:
      ClientObjHeroLogOn(@DefMsg, sBody);
    SM_HEROLOGON_OK:
      ClientObjHeroLogOnOK(DefMsg.Recog);

    SM_HEROLOGOUT:
      ClientObjHeroLogOut(@DefMsg);
    SM_HEROLOGOUT_OK:
      ClientObjHeroLogOutOK(DefMsg.Recog);

    SM_HEROABILITY:
      ClientObjHeroAbility(@DefMsg, sBody);
    SM_HEROSUBABILITY:
      ClientObjHeroSubAbility(@DefMsg);

    SM_HEROANGERVALUE:
      ClientObjFireDragonPoint(@DefMsg);
    SM_HEROBAGITEMS:
      ClientGetHeroBagItmes(@DefMsg, sBody);
    SM_HEROADDITEM:
      ClientGetHeroAddItem(sBody);
    SM_HERODELITEM:
      ClientGetHeroDelItem(sBody);
    SM_HERODELITEMS:
      ClientGetHeroDelItems(sBody);
    SM_HEROUPDATEITEM:
      ClientGetHeroUpdateItem(sBody);
    SM_HEROADDMAGIC:
      ClientGetHeroAddMagic(sBody);
    SM_SENDMYHEROMAGIC:
      ClientGetMyHeroMagics(sBody);
    SM_HERODELMAGIC:
      ClientGetHeroDelMagic(DefMsg.Recog);
    SM_HEROCHANGEITEM:
      ClientGetHeroChangeItem(sBody);

    SM_HEROTAKEON_OK:
      ClientObjHeroTakeOnOK(@DefMsg, DecodeString(sBody));
    SM_HEROTAKEON_FAIL:
      ClientObjHeroTakeOnFail();
    SM_HEROTAKEOFF_OK:
      ClientObjHeroTakeOffOK(@DefMsg, DecodeString(sBody));
    SM_HEROTAKEOFF_FAIL:
    ...
    SM_TAKEONITEMFROMHERO_OK:
      ClientObjTakeOnOK(@DefMsg, DecodeString(sBody));
    // 主人穿装备从英雄包裹   成功
    SM_TAKEONITEMFROMHERO_FAIL:
      ClientObjHeroTakeOnFail(); // 主人穿装备从英雄包裹 失败
    SM_TAKEOFFITEMTOHERO_OK:
      ClientObjTakeOffHeroBagOK(@DefMsg); // 主人脱装备到英雄包裹    成功
    SM_TAKEOFFITEMTOHERO_FAIL:
      ClientObjTakeOffHeroBagFail(); // 主人脱装备到英雄包裹  失败   }

    SM_HEROTAKEONITEMFROMMASTER_OK:
      ClientObjHeroTakeOnOK(@DefMsg, DecodeString(sBody));
    // 英雄穿装备从主人  成功
    SM_HEROTAKEONITEMFROMMASTER_FAIL:
      ClientObjTakeOnFail(); // 英雄穿装备从主人 失败
    SM_HEROTAKEOFFITEMTOMASTER_OK:
      ClientObjTakeOffMasterBagOK(@DefMsg); // 英雄脱装备到主人   成功
    SM_HEROTAKEOFFITEMTOMASTER_FAIL:
      ClientObjTakeOffMasterBagFail(); // 英雄脱装备到主人  失败

    SM_MASTERBAGTOHEROBAG_OK:
      ClientObjToHeroBagOK();
    SM_MASTERBAGTOHEROBAG_FAIL:
      ClientObjToHeroBagFail();
    SM_HEROBAGTOMASTERBAG_OK:
      ClientObjToMasterBagOK();
    SM_HEROBAGTOMASTERBAG_FAIL:
      ClientObjToMasterBagFail();

    SM_SENDHEROUSEITEMS:
      ClientGetSendHerouseItems(sBody);
    SM_HEROBAGCOUNT:
      ClientObjHeroBagCount(@DefMsg);
    SM_HEROWEIGHTCHANGED:
      ClientObjHeroWeigthChanged(@DefMsg);
    SM_HEROEAT_OK:
      ClientObjHeroEatOK();
    SM_HEROEAT_FAIL:
      ClientObjHeroEatFail();
    SM_HEROMAGIC_LVEXP:
      ClientGetHeroMagicLvExp(DefMsg.Recog { magid } , DefMsg.param
        { lv } , MakeLong(DefMsg.tag, DefMsg.series));
    SM_HERODURACHANGE:
      ClientGetHeroDuraChange(DefMsg.param
        { useitem index } , DefMsg.Recog, MakeLong(DefMsg.tag, DefMsg.series));
    SM_HEROWINEXP:
      ClientObjHeroWinExp(@DefMsg);
    SM_HEROLEVELUP:
      ClientObjHeroLevelUp(@DefMsg);
    SM_HERODROPITEM_SUCCESS:
      DelDropItem(DecodeString(sBody), DefMsg.Recog);
    SM_HERODROPITEM_FAIL:
      ClientGetHeroDropItemFail(DecodeString(sBody), DefMsg.Recog);
    SM_REPAIRFIRDRAGON_OK:
      g_WaitingUseItem.Item.S.Name := '';
    SM_REPAIRFIRDRAGON_FAIL:
      ClientObjRepaiFireDragon(@DefMsg);
    { ==================================英雄相关==================================== }
    SM_TAKEON_OK:
      ClientObjTakeOnOK(@DefMsg, DecodeString(sBody));
    SM_TAKEON_FAIL:
      ClientObjTakeOnFail();
    SM_TAKEOFF_OK:
      ClientObjTakeOffOK(@DefMsg, DecodeString(sBody));
    SM_TAKEOFF_FAIL:
      ClientObjTakeOffFail();
    SM_EXCHGTAKEON_OK:
      ;
    SM_EXCHGTAKEON_FAIL:
      ;
    SM_SENDUSEITEMS:
      ClientGetSenduseItems(sBody);
    SM_WEIGHTCHANGED:
      ClientObjWeigthChanged(@DefMsg);
    SM_GOLDCHANGED:
      ClientObjGoldChanged(@DefMsg);
    SM_FEATURECHANGED:
      PlayScene.SendMsg(DefMsg.ident, DefMsg.Recog, 0, 0, 0,
        MakeLong(DefMsg.param, DefMsg.tag), MakeLong(DefMsg.series, 0), '');
    SM_CHARSTATUSCHANGED:
      PlayScene.SendMsg(DefMsg.ident, DefMsg.Recog, 0, 0, 0,
        MakeLong(DefMsg.param, DefMsg.tag), DefMsg.series,
        DecodeString(sBody));
    SM_CLEAROBJECTS:
      ClientObjCleanObjects();
    SM_EAT_OK:
      ClientObjEatOK();
    SM_EAT_FAIL:
      ClientObjEatFail();
    SM_ADDMAGIC:
      ClientGetAddMagic(sBody);
    SM_SENDMYMAGIC:
      ClientGetMyMagics(sBody);
    SM_DELMAGIC:
      ClientGetDelMagic(DefMsg.Recog);
    SM_MAGIC_LVEXP:
      ClientGetMagicLvExp(DefMsg.Recog { magid } , DefMsg.param
        { lv } , MakeLong(DefMsg.tag, DefMsg.series));
    SM_DURACHANGE:
      ClientGetDuraChange(DefMsg.param
        { useitem index } , DefMsg.Recog, MakeLong(DefMsg.tag, DefMsg.series));
    SM_MERCHANTSAY:
      ClientGetMerchantSay(DefMsg.Recog, DefMsg.param, DecodeString(sBody));
    SM_MERCHANTDLGCLOSE:
      ClientObjCloseMDlg();
    SM_SENDGOODSLIST:
      ClientGetSendGoodsList(DefMsg.Recog, DefMsg.param, sBody);
    SM_SENDUSERMAKEDRUGITEMLIST:
      ClientGetSendMakeDrugList(DefMsg.Recog, sBody);
    SM_SENDUSERSELL:
      ClientGetSendUserSell(DefMsg.Recog);
    SM_SENDUSERREPAIR:
      ClientGetSendUserRepair(DefMsg.Recog);
    SM_SENDBUYPRICE:
      ClientObjBuyPrice(DefMsg.Recog);
    SM_USERSELLITEM_OK:
      ClientObjSellItemOK(DefMsg.Recog);
    SM_USERSELLITEM_FAIL:
      ClientObjSellItemFail();
    SM_SENDREPAIRCOST:
      ClientObjRepairCost(DefMsg.Recog);
    SM_USERREPAIRITEM_OK:
      ClientObjRepairItemOK(@DefMsg);
    SM_USERREPAIRITEM_FAIL:
      ClientObjRepairItemFail();
    SM_STORAGE_OK, SM_STORAGE_FULL, SM_STORAGE_FAIL:
      ClientObjStorageOK(@DefMsg);
    SM_SAVEITEMLIST:
      ClientGetSaveItemList(DefMsg.Recog, DefMsg.param, sBody);
    SM_TAKEBACKSTORAGEITEM_OK, SM_TAKEBACKSTORAGEITEM_FAIL,
      SM_TAKEBACKSTORAGEITEM_FULLBAG:
      ClientObjTakeBackStorageItemOK(@DefMsg);
    SM_BUYITEM_SUCCESS:
      ClientObjBuyItemSuccess(@DefMsg);
    SM_BUYITEM_FAIL:
      ClientObjBuyItemFail(DefMsg.Recog);
    SM_MAKEDRUG_SUCCESS:
      ClientObjMakeDrugOK(DefMsg.Recog);
    SM_MAKEDRUG_FAIL:
      ClientObjMakeDrugFail(DefMsg.Recog);
    SM_716:
      DrawEffectHum(DefMsg.series { type } , DefMsg.param { x } , DefMsg.tag
        { y } , DefMsg.Recog);
    SM_717:
      DrawEffectHum(DefMsg.param { type } , DefMsg.series, { x } DefMsg.tag
        { y } , DefMsg.Recog);
    SM_SENDDETAILGOODSLIST:
      ClientGetSendDetailGoodsList(DefMsg.Recog, DefMsg.param, DefMsg.tag,
        sBody);
    SM_TEST:
    ...
    SM_GROUPMODECHANGED:
      ClientObjGroupModeChanged(DefMsg.param);
    SM_CREATEGROUP_OK:
      ClientObjCreateGroupOK();
    SM_CREATEGROUP_FAIL:
      ClientObjCreateGroupFail(DefMsg.Recog);
    SM_GROUPADDMEM_OK:
      g_dwChangeGroupModeTick := GetTickCount;
    SM_GROUPADDMEM_FAIL:
      ClientObjGroupAddManFail(DefMsg.Recog);
    SM_GROUPDELMEM_OK:
      g_dwChangeGroupModeTick := GetTickCount;
    SM_GROUPDELMEM_FAIL:
      ClientObjGroupDelManFail(DefMsg.Recog);
    SM_GROUPCANCEL:
      g_GroupMembers.Clear;
    SM_GROUPMEMBERS:
      ClientGetGroupMembers(DecodeString(sBody));
    SM_OPENGUILDDLG:
      ClientGetOpenGuildDlg(sBody);
    SM_SENDGUILDMEMBERLIST:
      ClientGetSendGuildMemberList(sBody);
    SM_OPENGUILDDLG_FAIL:
      ClientObjOpenGuildDlgFail();
    SM_DEALTRY_FAIL:
      ClientObjDealtryFail();
    SM_DEALMENU:
      ClientObjDealMenu(sBody);
    SM_DEALCANCEL:
      ClientObjDealCancel();
    SM_DEALADDITEM_OK, SM_DEALADDITEM_FAIL:
      ClientObjDealAddItemOK();
    SM_DEALDELITEM_OK:
      ClientObjDealDelItemOK();
    SM_DEALDELITEM_FAIL:
      ClientObjDealDelItemFail();
    SM_DEALREMOTEADDITEM:
      ClientGetDealRemoteAddItem(sBody);
    SM_DEALREMOTEDELITEM:
      ClientGetDealRemoteDelItem(sBody);
    SM_DEALCHGGOLD_OK:
      ClientObjDealChgGoldOK(@DefMsg);
    SM_DEALCHGGOLD_FAIL:
      ClientObjDealChgGoldFail(@DefMsg);
    SM_DEALREMOTECHGGOLD:
      ClientObjDealRemotChgGold(@DefMsg);
    SM_DEALSUCCESS:
      FrmDlg.CloseDealDlg;
    SM_SENDUSERSTORAGEITEM:
      ClientGetSendUserStorage(DefMsg.Recog);
    SM_READMINIMAP_OK:
      ClientGetReadMiniMap(DefMsg.param);
    SM_READMINIMAP_FAIL:
      ClientGetReadMiniMapFail();
    SM_CHANGEGUILDNAME:
      ClientGetChangeGuildName(DecodeString(sBody));
    SM_SENDUSERSTATE:
      ClientGetSendUserState(sBody);
    SM_GUILDADDMEMBER_OK:
      SendGuildMemberList;
    SM_GUILDADDMEMBER_FAIL:
      ClientObjGuildAddMemberFail(DefMsg.Recog);
    SM_GUILDDELMEMBER_OK:
      SendGuildMemberList;
    SM_GUILDDELMEMBER_FAIL:
      ClientObjGuildDelMemberFail(DefMsg.Recog);
    SM_GUILDRANKUPDATE_FAIL:
      ClientObjGuildRankUpdateFail(DefMsg.Recog);
    SM_GUILDMAKEALLY_OK, SM_GUILDMAKEALLY_FAIL:
      ClientObjGuildMakeAllyFail(DefMsg.Recog);
    SM_GUILDBREAKALLY_OK, SM_GUILDBREAKALLY_FAIL:
      ClientObjGuildBreakAllyFail(DefMsg.Recog);
    SM_BUILDGUILD_OK:
      ClientObjBuildGuildOK();
    SM_BUILDGUILD_FAIL:
      ClientObjBuildGuildFail(DefMsg.Recog);
    SM_MENU_OK:
      ClientObjMenuOK(sBody);
    SM_DLGMSG:
      ClientObjDlgMsg(sBody);
    SM_DONATE_OK, SM_DONATE_FAIL:
      FrmDlg.LastestClickTime := GetTickCount;
    SM_PLAYDICE:
      ClientObjPalyDice(@DefMsg, sBody);
    SM_NEEDPASSWORD:
      ClientGetNeedPassword(sBody);
    SM_PASSWORDSTATUS:
      ClientGetPasswordStatus(@DefMsg, sBody);
    SM_SENGSHOPITEMS:
      ClientGetShopItems(@DefMsg, sBody); // 获取商铺物品
    // SM_QUERYUSERLEVELSORT: ClientGetSortItems(@DefMsg, sBody);
    SM_SENDUSERSELLOFFITEM:
      ClientGetSendUserSellOff(DefMsg.Recog);
    SM_SENDSELLOFFGOODSLIST:
      ClientGetSellItems(@DefMsg, sBody);
    SM_BUYSHOPITEM_FAIL:
      ClientObjBuyShopItemFail(DefMsg.Recog);
    SM_GETREGINFO:
      ClientGetRegInfo(@DefMsg, sBody);
    SM_SENDBUYSELLOFFITEM_OK:
      FrmDlg.DMessageDlg('Item bought', [mbOk]);
    SM_SENDBUYSELLOFFITEM_FAIL:
      ClientObjBuySellOffItemFail(DefMsg.Recog);
    SM_SENDUSERSELLOFFITEM_OK:
      ClientObjSellSellOffItemOK();
    SM_SENDUSERSELLOFFITEM_FAIL:
      ClientObjSellSellOffItemFail(DefMsg.Recog); // 寄售物品失败
    SM_USERCASTLE:
      ClientObjGetCastle(@DefMsg);
    SM_SHOWITEMBOX:
      ClientObjShowBox(DefMsg.Recog);
    SM_OPENITEMBOX_OK:
      ClientObjOpenBoxOK(@DefMsg, sBody);
    SM_OPENITEMBOX_FAIL:
      ClientObjOpenBoxFail(DefMsg.Recog);
    SM_GETSELBOXITEMNUM:
      ClientObjGetBoxIndex(DefMsg.Recog);

    SM_GETBACKITEMBOX_OK:
      ClientObjGetBackBoxOK(); // 取回宝箱成功
    // SM_GETBACKITEMBOX_FAIL: ; //取回宝箱失败

    SM_OPENBOOK:
      ClientObjOpenBook(DefMsg.Recog, DefMsg.param);
    SM_SENDCHANGEITEM:
      ClientGetSendUserChangeItem(DefMsg.Recog);
    SM_SENDCHANGEITEM_OK:
      ClientObjChangeItemOK(sBody);
    SM_SENDCHANGEITEM_FAIL:
      ClientObjChangeItemFail(DefMsg.Recog);

    SM_SENDUPGRADEITEM_OK:
      ClientObjUpgradeItemOK(sBody);
    SM_SENDUPGRADEITEM_FAIL:
      ClientObjUpgradeItemFail(DefMsg.Recog, sBody);

    SM_DUELTRY_FAIL:
      ClientObjDueltryFail(DefMsg.Recog);
    SM_DUELMENU:
      ClientObjDuelMenu(sBody);
    SM_DUELCANCEL:
      ClientObjDuelCancel();
    SM_DUELADDITEM_OK, SM_DUELADDITEM_FAIL:
      ClientObjDuelAddItemOK();
    SM_DUELDELITEM_OK:
      ClientObjDuelDelItemOK();
    SM_DUELDELITEM_FAIL:
      ClientObjDuelDelItemFail();
    SM_DUELREMOTEADDITEM:
      ClientGetDuelRemoteAddItem(sBody);
    SM_DUELREMOTEDELITEM:
      ClientGetDuelRemoteDelItem(sBody);
    SM_DUELCHGGOLD_OK:
      ClientObjDuelChgGoldOK(@DefMsg);
    SM_DUELCHGGOLD_FAIL:
      ClientObjDuelChgGoldFail(@DefMsg);
    SM_DUELREMOTECHGGOLD:
      ClientObjDuelRemotChgGold(@DefMsg);
    SM_DUELSUCCESS:
      FrmDlg.CloseDuelDlg;

    SM_STATE_BUBBLEDEFENCEUP:
      ClientObjChangeState(@DefMsg);
    SM_SENDOPENHOMEPAGE:
      if Assigned(g_PlugInfo.OpenHomePage) then
        g_PlugInfo.OpenHomePage(PChar(DecodeString(sBody)));

    SM_SENDFINDITEMINFO_OK:
      ClientObjFindItemOK(sBody, DefMsg.Recog);
    ...
    SM_SENDSTORE:
      ClientStore(@DefMsg);
    SM_DELSTOREITEM:
      ClientDelStoreItem(sBody);
    SM_USERSTOREITEMS:
      ClientGetSendUserStoreState(sBody);
    SM_SENDBUYSTOREITEM_OK:
      ClientObjBuyStoreItemOK(); // 摆摊物品购买成功
    SM_SENDBUYSTOREITEM_FAIL:
      ClientObjBuyStoreItemFail(@DefMsg); // 摆摊物品购买失败
    SM_SENDSTARTSTORE_OK:
      ClientObjStartStoreOK(); // 摆摊成功
    SM_SENDSTOPSTORE_OK:
      ClientObjStopStoreOK();
    SM_SENDSTARTSTORE_FAIL:
      ClientObjStartStoreFail(); // 摆摊成功

    SM_NEWSTATUS:
      ClientObjNewStatus(@DefMsg);

    SM_AUTOGOTOXY:
      ClientAutoGotoXY(DefMsg.param, DefMsg.tag, DecodeString(sBody));

    SM_TAKEONITEM:
      ClientTakeOnItem(DefMsg.Recog, DefMsg.param, DecodeString(sBody));
    // 穿装备
    SM_TAKEOFFITEM:
      ClientTakeOffItem(DefMsg.Recog, DefMsg.param, DecodeString(sBody));
    // 脱装备
    SM_HEROTAKEONITEM:
      ClientHeroTakeOnItem(DefMsg.Recog, DefMsg.param, DecodeString(sBody));
    // 穿装备
    SM_HEROTAKEOFFITEM:
      ClientHeroTakeOffItem(DefMsg.Recog, DefMsg.param, DecodeString(sBody));
    // 脱装备
    SM_PLAYSOUND:
      ClientPlaySound(DecodeString(sBody));
    SM_VIBRATION:
      ClientStartVibration(DefMsg.param);
    SM_OPENBIGDIALOGBOX:
      ClientOpenBigDiaLogBox(DefMsg.Recog);
    SM_CLOSEBIGDIALOGBOX:
      ClientCloseBigDiaLogBox();
    ...
    SM_STARTSERIESPELL_FAIL:
      StopSerieMagic;
    SM_BLASTHHIT:
      ClientObjBlasthit(@DefMsg);
    SM_SENDCARTINFO:
      ClientObjCartInfo(@DefMsg, DecodeString(sBody));
    SM_DELCARTINFO:
      ClientObjDelCartInfo(@DefMsg);
  else
    begin
...
procedure TfrmMain.ClientGetServerName(DefMsg: pTDefaultMessage; sBody: string);
var
  I: Integer;
  sServerName: string;
...
procedure TfrmMain.ClientGetPasswordOK(DefMsg: pTDefaultMessage; sBody: string);
var
...
procedure TfrmMain.ClientGetMapDescription(DefMsg: pTDefaultMessage;
  sBody: string);
var
  sTitle: string;
  sMapMusic: string;
...
procedure TfrmMain.ClientGetGameGoldName(DefMsg: pTDefaultMessage;
  sBody: string);
var
  sData: string;
begin
...
begin
  ...
  Loadbags('.\Data\' + g_sServerName + '.' + g_sSelChrName + '.itm',
    @ItemSaveArr);
...
procedure TfrmMain.ClientGetHeroChangeItem(body: string);
  ...
  begin
    ...
    begin
      if (g_HeroItemArr[I].S.Name = iname) and
        (g_HeroItemArr[I].MakeIndex = iindex) then
      begin
        Result := I;
        Break;
      end;
...
begin
  ...
  if (nWhere in [U_DRESS .. U_CHARM]) and (nMakeIndex <> 0) and
    (sItemName <> '') then
  begin
    nIndex := FindHeroItemBag(sItemName, nMakeIndex);
    if (nIndex >= 0) and (g_HeroItemArr[nIndex].S.Name <> '') then
    begin
      boFlag := False;
      ...
      if boFlag then
      begin
        if g_HeroUseItems[nWhere].S.Name <> '' then
        begin
...
procedure TfrmMain.ClientGetHeroBagItmes(DefMsg: pTDefaultMessage;
  body: string);
var
  sData, Str: string;
  cu: TClientItem;
...
procedure TfrmMain.ClientGetShopItems(DefMsg: pTDefaultMessage; body: string);
  ...
  var
    I: Integer;
  begin
    if ShopItem.StdItem.Name <> '' then
    begin
      if (ShopItem.btItemType >= 0) and (ShopItem.btItemType <= 4) then
      begin
        for I := 0 to 9 do
        begin
          if g_ShopItems[ShopItem.btItemType, I].StdItem.Name = '' then
          begin
      ...
      else if ShopItem.btItemType = 5 then
      begin
        for I := 0 to 4 do
        begin
          if g_SuperShopItems[I].StdItem.Name = '' then
          begin
...
begin
  sData := body;
  if DefMsg.series in [0 .. 5] then
  begin
    if DefMsg.series = 5 then
    begin
      for I := Low(g_SuperShopItems) to High(g_SuperShopItems) do
        g_SuperShopItems[I].StdItem.Name := '';
      // FillChar(g_SuperShopItems, SizeOf(TShopItem) * 5, #0);
    end
    else
    begin
      for I := Low(g_ShopItems[DefMsg.series]) to High
        (g_ShopItems[DefMsg.series]) do
        g_ShopItems[DefMsg.series, I].StdItem.Name := '';
...
procedure TfrmMain.ClientGetRanking(DefMsg: pTDefaultMessage; sBody: string);
  ...
  begin
    for I := 0 to 9 do
    begin
      if g_UserLevelRankings[I].sChrName = '' then
      begin
  ...
  begin
    for I := 0 to 9 do
    begin
      if g_HeroLevelRankings[I].sChrName = '' then
      begin
  ...
  begin
    for I := 0 to 9 do
    begin
      if g_UserMasterRankings[I].sChrName = '' then
      begin
    ...
    case g_nRankingsTablePage of
      ...
      2:
        begin
          ...
          SafeFillChar(g_UserMasterRankings, SizeOf(TUserMasterRanking) * 10,
            #0);
          while True do
          begin
            if sBody = '' then
...
procedure TfrmMain.ClientGetSellItems(DefMsg: pTDefaultMessage; body: string);
  ...
  var
    I: Integer;
  begin
    for I := Low(g_SellItems) to High(g_SellItems) do
    begin
      if (g_SellItems[I].sCharName = '') or
        (g_SellItems[I].SellItem.S.Name = '') then
      begin
        g_SellItems[I] := cu;
        Break;
      end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (cu.SellItem.S.Name <> '') and (cu.sCharName <> '') then
      begin
        AddSellItem(cu);
      end;
    end
...
procedure TfrmMain.ClientGetShowItem(DefMsg: pTDefaultMessage; sData: string);
var
  I, II: Integer;
  List: TList;
  DropItem: pTDropItem;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        if (pTDropItem(List.Items[0]).X = DefMsg.param) and
          (pTDropItem(List.Items[0]).Y = DefMsg.tag) then
        begin
          for II := 0 to List.Count - 1 do
          begin
            if pTDropItem(List.Items[II]).id = DefMsg.Recog then
            begin
              boFindItem := True;
              Break;
            end;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        DropItem := List.Items[II];
        if DropItem.id = DefMsg.Recog then
        begin
          List.Delete(II);
          Dispose(DropItem);
          if List.Count <= 0 then
...
begin
  ...
  g_MagicList.Add(pcm);
  FrmDlg.DComboboxAutoUseMagic.Items.AddObject(pcm.Def.sMagicName,
    TObject(pcm));
...
begin
  ...
  for I := 0 to g_SerieMagicList.Count - 1 do
  begin // 连击
    if PTClientMagic(g_SerieMagicList[I]).Def.wMagicId = magid then
    begin
      Dispose(PTClientMagic(g_SerieMagicList[I]));
  ...
  for I := 0 to g_MagicList.Count - 1 do
  begin
    FrmDlg.DComboboxAutoUseMagic.Items.AddObject
      (PTClientMagic(g_MagicList.Items[I]).Def.sMagicName,
      TObject(g_MagicList.Items[I]));
    FrmDlg.DComboboxGuajiQunti.Items.AddObject
      (PTClientMagic(g_MagicList.Items[I]).Def.sMagicName,
      TObject(g_MagicList.Items[I]));
    FrmDlg.DComboboxGuajiGeti.Items.AddObject
      (PTClientMagic(g_MagicList.Items[I]).Def.sMagicName,
      TObject(g_MagicList.Items[I]));
  end;

  if (g_SerieMagicList.Count > 0) and (FrmDlg.DSerieMagicMenu.Count <= 0) then
  begin
    FrmDlg.DSerieMagicMenu.Add('清除技能', nil);
    FrmDlg.DSerieMagicMenu.Add('随机选择', nil);
    FrmDlg.DSerieMagicMenu.Add('-', nil);
  ...
  for I := 0 to g_SerieMagicList.Count - 1 do
  begin
    FrmDlg.DSerieMagicMenu.Add(PTClientMagic(g_SerieMagicList.Items[I])
        .Def.sMagicName, nil);
  end;
  boSave := False;
  for I := 0 to 7 do
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        FrmDlg.DComboboxAutoUseMagic.Items.AddObject(pcm.Def.sMagicName,
          TObject(pcm));
        FrmDlg.DComboboxGuajiQunti.Items.AddObject(pcm.Def.sMagicName,
          TObject(pcm));
        FrmDlg.DComboboxGuajiGeti.Items.AddObject(pcm.Def.sMagicName,
          TObject(pcm));
      end;
    end
    else
...
procedure TfrmMain.ClientGetSaveItemList(merchant, Page: Integer;
  bodystr: string);
var
  I: Integer;
  Data, sMsg: string;
    ...
    begin
      ...
      begin
        ...
        if FrmDlg.GuildCommanderMode then
          FrmDlg.GuildStrs.AddObject(fmstr('(' + IntToStr(rank) + ')',
              3) + '<' + rankname + '>', TObject(clWhite))
        else
...
begin
  ...
  if g_FreeActorList <> nil then
  begin
    for I := g_FreeActorList.Count - 1 downto 0 do
    begin
      if GetTickCount - TActor(g_FreeActorList[I])
        .m_dwDeleteTime > 1000 * 20 then
      begin
...
begin
  ...
  case nType of
    ...
    1:
      Effect := TNormalDrawEffect.Create(nX, nY, g_WMonImages.Indexs[14], 410,
        6, 120, False);
    2:
      Effect := TNormalDrawEffect.Create(nX, nY, g_WMagic2Images, 670, 10, 150,
        False);
    3:
      begin
    ...
    12:
      begin // 流星火雨
        PlayScene.NewMagic(nil, 75, 51, nX, nY, nX, nY, 0, mtExplosion, False,
          30, bo15);
        PlaySound(10542);
      end;
    13:
      begin // 灵魂火符
        PlayScene.NewMagic(nil, 75, 51, nX, nY, nX, nY, nTarget, mtFly, False,
          30, bo15);
    ...
    14:
      begin // 破魂斩
        PlayScene.NewMagic(nil, 60, 60, nX, nY, nX, nY, nTarget, mtExplosion,
          False, 30, bo15);
        PlaySound(10512);
      end;
    ...
    15:
      begin // 灵魂火符
        PlayScene.NewMagic(nil, 13, 10, g_MySelf.m_nCurrX, g_MySelf.m_nCurrY,
          nX, nY, nTarget, mtExploBujauk, False, 30, bo15);
        PlaySound(10000 + 13 * 10 + 2);
    ...
    16:
      begin
        PlayScene.NewMagic(nil, 62, 62, nX, nY, nX, nY, 0, mtExplosion, False,
          30, bo15);
      end;
    17:
      begin
        PlayScene.NewMagic(nil, 64, 64, nX, nY, nX, nY, 0, mtFly, False, 30,
          bo15);
      end;
    18:
      begin
        PlayScene.NewMagic(nil, 65, 65, nX, nY, nX, nY, 0, mtExplosion, False,
          30, bo15);
      end;
    101:
      begin
        PlayScene.NewMagic(nil, 101, 101, nX, nY, nX, nY, 0, mtFly, False, 30,
          bo15);
      end;
    102:
      begin
        PlayScene.NewMagic(nil, 102, 102, nX, nY, nX, nY, 0, mtFly, False, 30,
          bo15);
      end;
  end;
  if Effect <> nil then
...
procedure TfrmMain.ClientGetServerConfig(DefMsg: pTDefaultMessage;
  sBody: string);
begin
  DecodeBuffer(sBody, @g_ServerConfig, SizeOf(TServerConfig));
  if g_ServerConfig.nMaxLevel <= 0 then
    g_ServerConfig.nMaxLevel := High(Word);
end;
...
begin
  ...
  begin
    ...
    begin
      DScreen.AddChatBoardString(Format('%s was not found', [sHumanName]),
        clWhite, clRed);
      Exit;
    end;
  end;
  ...
  begin
    case nIdent of
      ...
      SM_MAGICFIRE:
        Result := 'SM_MAGICFIRE';
      SM_CHANGENAMECOLOR:
        Result := 'SM_CHANGENAMECOLOR';
      SM_CHARSTATUSCHANGED:
        Result := 'SM_CHARSTATUSCHANGED';

      SM_SPACEMOVE_HIDE:
        Result := 'SM_SPACEMOVE_HIDE';
      SM_SPACEMOVE_SHOW:
        Result := 'SM_SPACEMOVE_SHOW';
      SM_SHOWEVENT:
        Result := 'SM_SHOWEVENT';
      SM_HIDEEVENT:
        Result := 'SM_HIDEEVENT';
...
var
  sLineText: string;
begin
  if (ShowMsgActor = nil) or (ShowMsgActor <> nil) and
    (ShowMsgActor.m_nRecogId = DefMsg.Recog) then
  begin
    sLineText := Format('ID:%d Ident:%s', [DefMsg.Recog,
      GetIdent(DefMsg.ident)]);
...
begin
  ...
  for I := 0 to g_ProcessesList.Count - 1 do
  begin
    if (CompareText(sName, g_ProcessesList.Strings[I]) = 0) then
    begin
      Result := True;
      Break;
    end;
...
function TfrmMain.GetProcesses: Boolean;
  ...
  begin
    ...
    begin
      ...
      begin
        with ModuleEntry do
          if th32ProcessID <> ProcessID then
          begin
            Result := True;
            Break;
          end;
  ...
  begin
    ...
    begin
      ...
      begin
        with ThreadEntry do
          if th32OwnerProcessID <> ProcessID then
          begin
            Result := True;
            Break;
          end;
  ...
  begin
    ...
    begin
      with ProcessEntry do
        if FindProcesses(StrPas(szEXEFile)) then
        begin
        ...
        begin
          if GetModules(th32ProcessID) then
          begin
            Result := True;
            Break;
          end;
...
begin
  ...
  begin
    ...
    begin
      with ModuleEntry do
        if FindProcesses(StrPas(szModule)) then
        begin
...
begin
  ...
  if GetTickCount - g_dwGetProcessesTick > 1000 * 10 then
  begin
  ...
  gcount := GetTickCount;
  if g_dwSHGetTime > 0 then
  begin
    // DScreen.AddChatBoardString(Format('timer:%d', [abs((gcount - g_dwSHGetTime) - (timer - g_dwSHTimerTime))]), clRed, clWhite);
    if abs((gcount - g_dwSHGetTime) - (timer - g_dwSHTimerTime)) > 50 then
    begin
      Inc(g_nSHFakeCount);
    end
    else
      g_nSHFakeCount := 0;
    if g_nSHFakeCount > 4 then
    begin
      CSocket.Active := False;
      SpeedHackTimer.Enabled := False;
...
begin
  ...
  if rtime > 0 then
  begin
    if g_nServer_ClientTime > 0 then
    begin
      ...
      if nServer_ClientTime > 1500 then
      begin
      ...
      if g_nTimeFakeDetectCount > 4 then
      begin
        CSocket.Active := False;
        SpeedHackTimer.Enabled := False;
...
begin
  if g_boClose then
    Exit;
  g_boCanDraw := False;
  if Assigned(g_PlugInfo.HookFinalize) then
  begin
...
begin
  ...
  nLen := nConfigClient + nClientOption + g_ConfigClient.nDataSize +
    g_ConfigClient.nBackBmpSize + g_ConfigClient.nBindItemSize +
    g_ConfigClient.nShowItemSize + g_ConfigClient.nItemDescSize;
  ...
  if (BufferCRC(InBuf, InBytes) = ClientOption.nCrc) then
  begin
    { ------------------------------------------------------------------------------- }
    ...
    if (BufferCRC(InBuf, InBytes) = g_ConfigClient.nCrc) then
    begin
      for I := 0 to 9 do
      begin
        if g_ConfigClient.PlugArr[I] <> '' then
        begin
      ...
      { ------------------------------------------------------------------------------- }
      if g_ConfigClient.nDataSize > 0 then
      begin
      ...
      if g_ConfigClient.nBackBmpSize > 0 then
      begin
      ...
      { ------------------------------------------------------------------------------- }
      if g_ConfigClient.nBindItemSize > 0 then
      begin
      ...
      { ------------------------------------------------------------------------------- }
      if g_ConfigClient.nShowItemSize > 0 then
      begin
      ...
      { ------------------------------------------------------------------------------- }
      if g_ConfigClient.nItemDescSize > 0 then
      begin
...
procedure TfrmMain.HTTPGetCheckProDoneString(Sender: TObject; Result: string);
var
  I: Integer;
begin
...
begin
  ...
  if (LegendMap.EndX > 0) and (LegendMap.EndY > 0) then
  begin
    if (abs(g_MySelf.m_nCurrX - LegendMap.EndX) <= 1) and
      (abs(g_MySelf.m_nCurrY - LegendMap.EndY) <= 1) then
    begin
      DScreen.AddChatBoardString(Format(
          'You have reached the location at (%d:%d).', [LegendMap.EndX,
          LegendMap.EndY]), GetRGB(154), clWhite);
      LegendMap.Stop;
    end
    else
    begin
      if GetTickCount - g_dwAutoFindPathTick > 200 then
      begin
        ...
        if (Length(LegendMap.RunPath) <= 0) and (not LegendMap.StartFind) and
          (LegendMap.FindCount = 0) then
        begin
          LegendMap.Find(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, LegendMap.EndX,
            LegendMap.EndY);
          Exit;
        end;
        ...
        begin
          DScreen.AddChatBoardString(Format(
              'Unable to reach location at (%d:%d)', [LegendMap.EndX,
              LegendMap.EndY]), GetRGB(154), clWhite);
          LegendMap.Stop;
        end
        else
        begin
          if (LegendMap.PathPositionIndex >= 0) and
            (LegendMap.PathPositionIndex < Length
              (LegendMap.RunPath)) then
          begin
            if g_nAutoWalkX < 0 then
            begin
              g_nAutoWalkX := g_MySelf.m_nCurrX;
              g_nAutoWalkY := g_MySelf.m_nCurrY;
            end;

            if (g_MySelf.m_nCurrX = g_nAutoWalkX) and
              (g_MySelf.m_nCurrY = g_nAutoWalkY) then
            begin
              ...
              if (abs(g_MySelf.m_nCurrX - pt.X) <= 1) and
                (abs(g_MySelf.m_nCurrY - pt.Y) <= 1) then
              begin
            ...
            begin
              ...
              if GetTickCount - g_dwAutoWalkTick > 2000 then
              begin
                g_dwAutoWalkTick := GetTickCount;
                ...
                if g_nAutoWalkErrorCount < 3 then
                begin
                  LegendMap.Find(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY,
                    LegendMap.EndX, LegendMap.EndY);
                  LegendMap.PathPositionIndex := 0;
                end
                else
                begin
...
begin
  if (g_EatingItem.S.Name = '') and (g_WaitingUseItem.Item.S.Name = '') then
  begin
    if { (g_MovingItem.Item.s.Name <> '') } g_boItemMoving and
      (g_MovingItem.Owner = FrmDlg.DItemBag) and
      (GetTickCount - g_dwMoveItemTick > 100) then
    begin
      for I := Low(g_OldItemArr) to High(g_OldItemArr) do
      begin
        if (g_ItemArr[I].S.Name <> g_OldItemArr[I].S.Name) then
        begin
    ...
    if (GetTickCount - g_dwAutoOrderItemTick > 500)
    { and (GetTickCount - g_dwMoveItemTick > 100) } and not g_boItemMoving then
    begin
      ...
      for I := Low(g_OldItemArr) to High(g_OldItemArr) do
      begin
        if (g_ItemArr[I].S.Name = '') and (g_OldItemArr[I].S.Name <> '') then
        begin
          boFind := False;
          for II := Low(g_ItemArr) + 6 to High(g_ItemArr) do
          begin
            if g_ItemArr[II].S.Name <> '' then
            begin
              if (g_ItemArr[II].S.Name = g_OldItemArr[I].S.Name) then
              begin
...
begin
  Result := False;
  if (idx in [0 .. 46 - 1]) and (g_WaitingUseItem.Item.S.Name = '') then
  begin
    if (g_EatingItem.S.Name <> '') and (GetTickCount - g_dwEatTime > 5 * 1000)
      then
    begin
    ...
    begin
      if (g_ItemArr[idx].S.Name <> '') and ((g_ItemArr[idx].S.StdMode <= 3) or
          (g_ItemArr[idx].S.StdMode = 31)) then
      begin
        g_dwEatTime := GetTickCount;
        g_EatingItem := g_ItemArr[idx];
        g_ItemArr[idx].S.Name := '';
...
begin
  Result := False;
  if (idx in [0 .. 40 - 1]) and (g_WaitingUseItem.Item.S.Name = '') then
  begin
    if (g_HeroEatingItem.S.Name <> '') and
      (GetTickCount - g_dwHeroEatTime > 5 * 1000) then
    begin
    ...
    if (g_HeroEatingItem.S.Name = '') then
    begin
      if (g_HeroItemArr[idx].S.Name <> '') and
        ((g_HeroItemArr[idx].S.StdMode <= 3) or
...
var
  ...
  begin
    Result := False;
    if (g_Config.nRenewHumHPIndex1 >= 0) and
      (GetTickCount - g_dwRenewHumHPTick1 >
        g_Config.nRenewHumHPTime1) and
      (flag or (g_MySelf.m_Abil.HP < g_Config.nRenewHumHPPercent1)) then
    begin
  ...
  begin
    Result := False;
    if (g_Config.nRenewHumHPIndex2 >= 0) and
      (GetTickCount - g_dwRenewHumHPTick2 >
        g_Config.nRenewHumHPTime2) and
      (flag or (g_MySelf.m_Abil.HP < g_Config.nRenewHumHPPercent2)) then
    begin
...
begin
  if (g_WaitingUseItem.Item.S.Name = '') and (g_MySelf <> nil) and
    (not g_MySelf.m_boDeath) and (g_MySelf.m_Abil.HP > 0) then
  begin
    if g_Config.boRenewHumHPIsAuto1 and g_Config.boRenewHumHPIsAuto2 then
    begin
      if g_Config.nRenewHumHPPercent2 < g_Config.nRenewHumHPPercent1 then
      begin
        if (g_MySelf.m_Abil.HP < g_Config.nRenewHumHPPercent2) then
        begin
          ...
          begin
            if (g_MySelf.m_Abil.HP < g_Config.nRenewHumHPPercent1) then
            begin
              if not EatHPItem1(False) then
                if not EatHPItem2(False) then
            ...
            else if g_Config.boRenewHumHPIsAuto1 and
              (not g_Config.boRenewHumHPIsAuto2) then
            begin
              EatHPItem1(False);
            end
            else if (not g_Config.boRenewHumHPIsAuto1)
              and g_Config.boRenewHumHPIsAuto2 then
            begin
              EatHPItem2(False);
            end;
          end;
        ...
        var
          ...
          begin
            Result := False;
            if (g_Config.nRenewHumMPIndex1 >= 0) and
              (GetTickCount - g_dwRenewHumMPTick1 > g_Config.nRenewHumMPTime1)
              and (flag or (g_MySelf.m_Abil.MP < g_Config.nRenewHumMPPercent1))
              then
            begin
          ...
          begin
            Result := False;
            if (g_Config.nRenewHumMPIndex2 >= 0) and
              (GetTickCount - g_dwRenewHumMPTick2 > g_Config.nRenewHumMPTime2)
              and (flag or (g_MySelf.m_Abil.MP < g_Config.nRenewHumMPPercent2))
              then
            begin
        ...
        begin
          if (g_WaitingUseItem.Item.S.Name = '') and (g_MySelf <> nil) and
            (not g_MySelf.m_boDeath) { and (g_MySelf.m_Abil.MP > 0) } then
          begin
            if g_Config.boRenewHumMPIsAuto1 and g_Config.boRenewHumMPIsAuto2
              then
            begin

              if g_Config.nRenewHumMPPercent2 <
                g_Config.nRenewHumMPPercent1 then
              begin
                if (g_MySelf.m_Abil.MP < g_Config.nRenewHumMPPercent2) then
                begin
                  ...
                  begin
                    if (g_MySelf.m_Abil.MP < g_Config.nRenewHumMPPercent1) then
                    begin
                      if not EatMPItem1(False) then
                        if not EatMPItem2(False) then
                    ...
                    else if g_Config.boRenewHumMPIsAuto1 and
                      (not g_Config.boRenewHumMPIsAuto2) then
                    begin
                      EatMPItem1(False);
                    end
                    else if (not g_Config.boRenewHumMPIsAuto1)
                      and g_Config.boRenewHumMPIsAuto2 then
                    begin
                      EatMPItem2(False);
                    end;
                  end;
                ...
                var
                  ...
                  begin
                    Result := False;
                    if (GetTickCount - g_dwRenewBookTick >
                        g_Config.nRenewBookTime * 1000) and
                      (flag or
                        (g_MySelf.m_Abil.HP <
                          g_Config.nRenewBookPercent)) then
                    begin
                ...
                begin
                  if g_Config.boRenewBookIsAuto and
                    (g_Config.sRenewBookNowBookItem <> '') and
                ...
                var
                  ...
                  begin
                    Result := False;
                    if (g_Config.nRenewHeroHPIndex1 >= 0) and
                      (GetTickCount - g_dwRenewHeroHPTick1 >
                        g_Config.nRenewHeroHPTime1) and
                      (flag or (g_MyHero.m_Abil.HP <
                          g_Config.nRenewHeroHPPercent1)) then
                    begin
                  ...
                  begin
                    Result := False;
                    if (g_Config.nRenewHeroHPIndex2 >= 0) and
                      (GetTickCount - g_dwRenewHeroHPTick2 >
                        g_Config.nRenewHeroHPTime2) and
                      (flag or (g_MyHero.m_Abil.HP <
                          g_Config.nRenewHeroHPPercent2)) then
                    begin
                ...
                begin
                  if (g_WaitingUseItem.Item.S.Name = '') and (g_MyHero <> nil)
                    and (not g_MyHero.m_boDeath) and (g_MyHero.m_Abil.HP > 0)
                    then
                  begin
                    if (g_Config.boRenewHeroHPIsAuto1) and
                      (g_Config.boRenewHeroHPIsAuto2) then
                    begin

                      if g_Config.nRenewHeroHPPercent2 <
                        g_Config.nRenewHeroHPPercent1 then
                      begin
                        if (g_MyHero.m_Abil.HP < g_Config.nRenewHeroHPPercent2)
                          then
                        begin
                          if not EatHPItem2(False) then
                            if not EatHPItem1(False) then
                          ...
                          begin
                            ...
                            else if (g_Config.boRenewHeroHPIsAuto1) and
                              (not g_Config.boRenewHeroHPIsAuto2) then
                            begin
                              EatHPItem1(False);
                            end
                            else if (not g_Config.boRenewHeroHPIsAuto1) and
                              (g_Config.boRenewHeroHPIsAuto2) then
                            begin
                              EatHPItem2(False);
                            end;
                        ...
                        var
                          ...
                          begin
                            Result := False;
                            if (g_Config.nRenewHeroMPIndex1 > 0) and
                              (GetTickCount - g_dwRenewHeroMPTick1 >
                                g_Config.nRenewHeroMPTime1) and
                              (flag or
                                (g_MyHero.m_Abil.MP <
                                  g_Config
                                  .nRenewHeroMPPercent1)) then
                            begin
                          ...
                          begin
                            Result := False;
                            if (g_Config.nRenewHeroMPIndex2 > 0) and
                              (GetTickCount - g_dwRenewHeroMPTick2 >
                                g_Config.nRenewHeroMPTime2) and
                              (flag or
                                (g_MyHero.m_Abil.MP <
                                  g_Config
                                  .nRenewHeroMPPercent2)) then
                            begin
                        ...
                        begin
                          if (g_WaitingUseItem.Item.S.Name = '') and
                            (g_MyHero <> nil) and
                            (not g_MyHero.m_boDeath)
                          { and (g_MyHero.m_Abil.MP > 0) } then
                          begin
                            if g_Config.boRenewHeroMPIsAuto1 and g_Config.
                              boRenewHeroMPIsAuto2 then
                            begin
                              if g_Config.nRenewHeroMPPercent2 <
                                g_Config.nRenewHeroMPPercent1 then
                              begin
                                if
                                  (g_MyHero.m_Abil.MP <
                                    g_Config
                                ...
                                begin
                                  ...
                                  begin
                                    ...
                                    else if g_Config.boRenewHeroMPIsAuto1 and
                                      (not g_Config.boRenewHeroMPIsAuto2) then
                                    begin
                                      EatMPItem1(False);
                                    end
                                    else if (not g_Config.boRenewHeroMPIsAuto1)
                                      and g_Config.boRenewHeroMPIsAuto2 then
                                    begin
                                      EatMPItem2(False);
                                    end;
                                  end;
                                ...
                                var
                                  ...
                                  sHint: string;
                                begin
                                  if g_Config.boDuraWarning and
                                    (g_MySelf <> nil)
                                  { and (not g_MySelf.m_boDeath) } then
                                  begin
                                    if GetTickCount - g_dwHintItemDuraTick >
                                      1000 * 100 then
                                    begin
                                      ...
                                      begin
                                        ...
                                        begin
                                        if g_UseItems[I].Dura <= Round
                                        (g_UseItems[I].DuraMax * 25 / 100) then
                                        begin
                                        sHint := g_UseItems[I].S.Name +
                                        ' durability is low! Your item may break if Durability reaches 0';

                                        // if not PlayScene.EdChat.Visible then begin
                                        DScreen.AddSysMsg(sHint,
                                        SCREENWIDTH - 150, 300, clyellow);
                                        // else
                                        DScreen.AddChatBoardString(sHint,
                                        clyellow, clRed);
                                        end;
                                        end;
                                      end;
                                ...
                                begin
                                  ...
                                  if (g_MySelf <> nil) and
                                    (g_ConnectionStep = cnsPlay) then
                                  begin
                                    if mrOk = FrmDlg.DMessageDlg
                                      (
                                      'Are you sure you want to exit the game?', [mbOk, mbCancel]) then
                                    begin
                                      if
                                        (GetTickCount -
                                        g_dwLatestStruckTick > 10000) and
                                        (GetTickCount - g_dwLatestMagicTick >
                                        10000) and
                                        (GetTickCount - g_dwLatestHitTick >
                                        10000) or (g_MySelf.m_boDeath) then
                                      begin

                                        if g_boBagLoaded then
                                        Savebags
                                        ('.\Data\' + g_sServerName + '.' +
                                        g_sSelChrName + '.itm', @g_ItemArr);
                                        g_boBagLoaded := False;
                                        SendGameCenterMsg(CM_QUIT,
                                        IntToStr(Handle));
                                      end
                                      else
                                      begin
                                        CanClose := False;
                                        DScreen.AddChatBoardString
                                        ('You cannot Exit the game during Battle.'
                                        , clyellow, clRed);
                                      end;
                                    end;
                                  end
                                ...
                                procedure TfrmMain.TimerPlayMediaTimer
                                  (Sender: TObject);
                                var
                                  I: Integer;
                                begin
                                  if GetTickCount - g_dwPlayMediaTick >
                                    g_dwPlayMediaTime then
                                  begin
                                    g_dwPlayMediaTick := GetTickCount;
                                    g_MediaList.Lock;
                                    try
                                      for I := 0 to g_MediaList.Count - 1 do
                                      begin
                                        try
                                        if Assigned
                                        (g_PlugInfo.MediaPlayer.Player) then
                                        begin
                                        g_PlugInfo.MediaPlayer.Player
                                        (PChar(g_MediaList[I]), False, True);
                                        end;
                                        except
                                        end;
                                      end;
                                ...
                                procedure TfrmMain.ClientStartVibration
                                  (nCount: Integer);
                                var
                                  ...
                                  begin
                                    ...
                                    begin
                                      ...
                                      begin
                                        ...
                                        g_VibrationValue[nLen].Y :=
                                        g_VibrationValue[nLen].Y - nValue div Y;

                                        Inc(nLen);
                                        SetLength(g_VibrationValue, nLen + 1);
                                        g_VibrationValue[nLen].X :=
                                        g_VibrationValue[nLen].X - nValue div Y;
                                ...
                                procedure TfrmMain.ClientOpenBigDiaLogBox
                                  (nImageIndex: Integer);
                                var
                                  I: Integer;
                                  c: TControl;
                                begin
                                  ...
                                  for I :=
                                    FrmDlg.DMerchantBigDlg.DControls.Count -
                                    1 downto 0 do
                                  begin
                                    c := TControl
                                      (FrmDlg.DMerchantBigDlg.DControls.Items
                                        [I]);
                                    if (c is TDLabel) or (c is TNpcLabel) then
                                    begin
                                      c.Free;
                                    end;
                                  end;
                                ...
                                procedure TfrmMain.tmrHeroLogoutTimer
                                  (Sender: TObject);
                                begin
                                  frmMain.SendClientMessage(CM_HEROLOGOUT,
                                    g_MyHero.m_nRecogId, 0, 0, 0);

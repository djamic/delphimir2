...
begin
  ...
  SendGameCenterMsg(SG_STARTNOW, 'Cái gì...');
...
begin
  if m_boRemoteClose then
    exit;
  if Application.MessageBox('Bạn thực sự muốn thoát chương trình?', 'Cảnh báo',
...
begin
  ...
  sLogDir := sBaseDir + IntToStr(Year) + '-' + IntToString(Month)
    + '-' + IntToString(Day);
  if not DirectoryExists(sLogDir) then
  begin
    CreateDirectoryA(PAnsiChar(sLogDir), nil);
  end;
  sLogFile := sLogDir + '\Log-' + IntToString(Hour) + 'h' + IntToString
    ((Min div 10) * 2) + 'm.txt';
  ...
  try
    ...
    begin
      Writeln(F, LogMsgList.Strings[I] + #9 + FormatDateTime

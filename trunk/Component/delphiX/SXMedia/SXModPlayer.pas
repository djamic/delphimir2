...
uses
  ...
  DXSounds, DirectX, MMSystem,MpSndSys;
...
var Msg : TMsg;
   ...
   begin
     ...
     if SoundBuffer.IBuffer.Lock(Pos,NumWrite,w1,s1,Pointer(nil^),s2,0) = 0 then
     begin
...
begin
    CurrentEvent := MsgWaitForMultipleObjects(EVENTCOUNT,Events.List^[0], False, INFINITE, QS_ALLINPUT);
...
begin
   ...
   begin
      ...
      if Result.SetNotificationPositions(EVENTCOUNT,PNotify[0]) <> 0 then
...
begin
  Data := GetSilenceData;
  if SoundBuffer.IBuffer.Lock(0,0,w1,s1,w2,s2,DSBLOCK_ENTIREBUFFER) = 0 then
  begin

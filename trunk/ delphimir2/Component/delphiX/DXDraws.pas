...
type
  ...
  var
    ...
    begin
      ...
      rec.D3DFlag := False;
      try
        Result := {$IFDEF D3D_deprecated}Direct3D{$ELSE}Direct3D7{$ENDIF}.EnumDevices(@EnumDeviceCallBack, rec) = DD_OK
      except
        on E: Exception do
        begin
          rec.D3DFlag := False;
...
begin
  ...
  try
    ...
    try
      ...
      {$IFDEF DEBUG} {T.VDIB.SaveToFile('c:\test_image.dib');} {for debug} {$ENDIF}
      if dds.Modified then

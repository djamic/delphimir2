...
{$IFEND}
  ...
  begin
    ...
    begin
      Caption := g_Config.TitleName + ' [Đang đóng...]';
      g_boClose := True;
      TimerStart.Enabled := True;
      CanClose := False;
    end
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        // StatusPane5.Caption := Format('¹¤×÷Ïß³Ì:%d', [g_dwDecodeMsgTime]);
        { ClientSocket.Port := g_Config.ServerPort;
          ClientSocket.Address := g_Config.ServerAddr;
          ClientSocket.Active := True; }
      end
...
begin
  ...
  begin
    ...
    begin
      ...
      MainOutMessage('chưa dịch được', nil);
      Exit;
    end;
    ...
    begin
      if not boIsUserIPList then
      begin
        AObject := UpdateCCAttackIPList(sRemoteIPaddr); { CC¹¥»÷¼ì²â }
        ...
        begin
          ...
          begin // CC¹¥»÷
            ...
            Inc(BlockIP.nAttackCount);
            MainOutMessage('CC gì đây: ' + sRemoteIPaddr, TObject(BlockIP));

...
begin
  {if OpenDialog.Execute and (OpenDialog.FileName <> '') then begin
   ...
   // Showmessage(IntToStr(CurrImageFile.Count));
    ...
    //StatusBar1.Panels[0].Text := OpenDialog1.FileName + ', Í¼Æ¬ÊýÁ¿:' + IntTosTr(CurrIdsFile.ImageCount);
    //bChanged := false;
  end; }
end;
...
begin
  ...
  if (Index >= 0) and (Index < CurrImageFile.ImageCount) then begin
    ...
    StatusPane3.Caption := Format('STT:%.5d Cao:%d Rộng:%d Vị trí:%d,%d', [Index, nImageWidth, nImageHeight, nImageX, nImageY]);
  end;
end;
        ...
        if frmInPutDlg.RadioGroup.ItemIndex = 0 then begin
          ...
          if Index < CurrImageFile.ImageCount then begin
            if CurrImageFile.Replace(Index, Source) then begin //Ìæ»»Í¼Æ¬
              Application.MessageBox('Đã hoàn thành việc thêm ảnh', 'Cảnh báo!', MB_ICONQUESTION);
            end else Application.MessageBox('Có lỗi, quá nhiều ảnh!', 'Cảnh báo!', MB_ICONQUESTION);
          end;
        end else
        ...
        if Execute and (FileName <> '') then begin
          ...
          if Bitmap <> nil then begin
            try
              ...
              Application.MessageBox('Hoàn thành lưu ảnh', 'Cảnh báo!', MB_ICONQUESTION);
            except
              Application.MessageBox('Có lỗi, chưa hoàn thành lưu ảnh', 'Cảnh báo!', MB_ICONQUESTION);
            end;
          end;
        end;
      end;
  ...
  try
    if FrmCreateData.ShowModal = 10 then begin
      with SaveDialog2 do begin
        ...
        case ImageType of
          t_Wil: begin
              Filter := 'Dữ liệu dạng WIL|*.Wil|*.Wil';
          ...
          t_Fir: begin
              BitCount := 16;
              Filter := 'Dữ liệu dạng DATA|*.Data|*.Data';
        ...
        if Execute and (FileName <> '') then begin
          ...
          if MirImages.CreateFile(BitCount) then
            Application.MessageBox('Tạo thành công', 'Cảnh báo!', MB_ICONQUESTION)
          else
            Application.MessageBox('Có lỗi, không tạo thành công', 'Cảnh báo!', MB_ICONQUESTION);
          MirImages.Free;
        end;
      end;
    end;
  ...
  try
    ...
    if Index < CurrImageFile.ImageCount then begin
      ...
      if frmXYDlg.ShowModal = 10 then begin
        ...
        Application.MessageBox('Mở quá nhiều', 'Cảnh báo!', MB_ICONQUESTION);
      end;
    end;
  finally
    g_boWalking := False;
...
{$IFEND}
    if frmConvertDlg.RadioGroup.ItemIndex = 1 then begin
      ...
      case GetWilFileVersion(ShortName) of
        -1: begin
            Application.MessageBox('¸ÃWILÎÄ¼þÒÑËð»µ £¡£¡£¡', 'Cảnh báo!', MB_ICONQUESTION);
            g_boWalking := False;
            Exit;
          end;
  ...
  if boConvert then Application.MessageBox('×ª»»³É¹¦ £¡£¡£¡', 'Cảnh báo!', MB_ICONQUESTION)
  else Application.MessageBox('×ª»»Ê§°Ü £¡£¡£¡', 'Cảnh báo!', MB_ICONQUESTION);
  g_boWalking := False;
  *)

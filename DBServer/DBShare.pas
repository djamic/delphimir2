...
begin
  if not FileExists(g_sServerIPConfFileNmae) then begin
    ...
    try
      ...
      MainOutMessage('Lỗi xảy ra khi đọc tệp: ' + g_sServerIPConfFileNmae + ' . Hãy kiểm tra lại');
    end;
  end;
end;
...
begin
  ...
  for I := 1 to Length(sChrName) do begin
    ...
    if boIsTwoByte then begin
      //if Chr < #$A1 then Result:=False; //Èç¹ûÐ¡ÓÚ¾ÍÊÇ·Ç·¨×Ö·û
//      if Chr < #$81 then Result:=False; //Èç¹ûÐ¡ÓÚ¾ÍÊÇ·Ç·¨×Ö·û

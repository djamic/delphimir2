...
type
  ...
  end{$IFDEF VER9UP}deprecated{$ENDIF};
  ...
  if Image = nil then
    if AsSigned(FOnGetImage) then begin
      ...
      if FDXImageName <> '' then
        if Assigned(FDXImageList) then begin
...
begin
  if (FBlurImage <> Value) and (Value) then begin
      ...
      begin
        ...
        if PatternIndex >= 0 then begin
          if AsSigned(FOnDraw) then
            ...
            {New function implemented}
            if Assigned(FEngine.FOwner) then begin
              Q := Map[cx2,cy2];
...
begin
  ...
  if Assigned(FMap) and (Header.MapWidth > 0) and (Header.MapHeight > 0) then begin

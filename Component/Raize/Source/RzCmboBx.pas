...
begin
  Result := SendTextMessage( Handle, cb_FindString, -1, S );
end;
...
procedure TRzCustomComboBox.DoSetTextHint;
var
  WS: WideString;
begin
  if ThemeServices.ThemesEnabled and HandleAllocated then
  begin
    WS := FTextHint;
    if RunningAtLeast( winVista ) then
      SendMessage( Handle, CB_SETCUEBANNER, WParam( 0 ), LParam( PWideChar( WS ) ) )
    else if RunningAtLeast( winXP ) then
      SendMessage( FEditHandle, EM_SETCUEBANNER, WParam( 0 ), LParam( PWideChar( WS ) ) );
  end;
end;


procedure TRzCustomComboBox.SetTextHint( const Value: string );
begin
...
begin
  ...
  if FFrameVisible and not UseThemes and not ( RunningAtLeast( WinVista ) and ThemeServices.ThemesEnabled ) then
  begin
    // Erase Ctl3D Border
    if ThemeServices.ThemesEnabled then
    begin
  ...
  if ThemeServices.ThemesEnabled and ( Style <> csSimple ) and
     ( RunningUnder( WinXP ) or
       ( RunningUnder( WinVista ) and ( Style = csDropDown ) ) ) then
  begin
    // Fill in the gap between the edit area and the dropdown button in XP themes
    R := BtnRect;
    if not UseRightToLeftAlignment then
    begin
      R.Right := R.Left;
  ...
  if FFlatButtons and not ( RunningAtLeast( WinVista ) and ThemeServices.ThemesEnabled ) and not FReadOnly then
  begin
...
begin
  ...
  PaintIt := ( FFlatButtons and not ( RunningAtLeast( WinVista ) and ThemeServices.ThemesEnabled ) ) or FFrameHotTrack;

...
begin
  ...
  begin
    ...
    StrPCopy( ( PAnsiChar( ptrDROPFILES ) + CurPos ), FileName );
...
begin
  ...
  PathStr := AnsiString( aPathname );
  mhPendingChangeHandler := _FindFirstChangeNotification( PAnsiChar( PathStr ), lookup[ afRecursive ], adwFlags );
  if ( mhPendingChangeHandler <> INVALID_HANDLE_VALUE ) then
  begin

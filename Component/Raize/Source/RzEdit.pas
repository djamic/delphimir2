...
{$ENDIF}
begin
  if RunningAtLeast( winXP ) and ThemeServices.ThemesEnabled and HandleAllocated then
  begin
    {$IFNDEF UNICODE}
    WS := Value;
...
procedure TRzCustomEdit.SetTextHint( const Value: string );
begin
...
begin
  ...
  begin
    Result := CharInSet( Key, [ '0'..'9', 'E', 'e', '+', '-', AnsiChar( DecimalSeparator ), '(', ')' ] )
              or
              ( ( Key < #32 ) and ( Key <> Chr( vk_Return ) ) );
  end
  else
  begin
    Result := CharInSet( Key, [ '0'..'9', '+', '-', AnsiChar( DecimalSeparator ), '(', ')' ] )
              or
              ( ( Key < #32 ) and ( Key <> Chr( vk_Return ) ) );
  end;
  ...
  begin
    ...
    begin
      ...
      if FAllowScientificNotation then
        IncludeChar := CharInSet( C, [ '0'..'9', 'E', 'e', '-', AnsiChar( DecimalSeparator ) ] )
      else
        IncludeChar := CharInSet( C, [ '0'..'9', '-', AnsiChar( DecimalSeparator ) ] );
  ...
  try
    ...
    begin
      ...
      try
        ...
        // size of the calculator. Of course, would need to be more flexible.
        Calculator.AutoSize := False;
        Calculator.Width := 200;
        Calculator.Height := 300;
        *)
...
begin
  ...
  begin
    ...
    KeepInvalidText := False;
    NewTime := 0;
    if not StrToTimeEx( Text, DT ) then
    begin

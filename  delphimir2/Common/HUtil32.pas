...
uses
  Classes, SysUtils, StrUtils, WinProcs, Graphics, Messages, Dialogs
...
const
  MAXDEFCOLOR = 16;
  ColorNames: array [1 .. MAXDEFCOLOR] of TyNameTable = ((Name: 'BLACK';
      varl: clBlack), (Name: 'BROWN'; varl: clMaroon), (Name: 'MARGENTA';
      varl: clFuchsia), (Name: 'GREEN'; varl: clGreen), (Name: 'LTGREEN';
      varl: clOlive), (Name: 'BLUE'; varl: clNavy), (Name: 'LTBLUE';
      varl: clBlue), (Name: 'PURPLE'; varl: clPurple), (Name: 'CYAN';
      varl: clTeal), (Name: 'LTCYAN'; varl: clAqua), (Name: 'GRAY';
      varl: clGray), (Name: 'LTGRAY'; varl: clSilver), (Name: 'YELLOW';
      varl: clYellow), (Name: 'LIME'; varl: clLime), (Name: 'WHITE';
      varl: clWhite), (Name: 'RED'; varl: clRed));

  MAXLISTMARKER = 3;
  LiMarkerNames: array [1 .. MAXLISTMARKER] of TyNameTable = ((Name: 'DISC';
      varl: 0), (Name: 'CIRCLE'; varl: 1), (Name: 'SQUARE'; varl: 2));

  MAXPREDEFINE = 3;
  PreDefineNames: array [1 .. MAXPREDEFINE] of TyNameTable = ((Name: 'LEFT';
      varl: 0), (Name: 'RIGHT'; varl: 1), (Name: 'CENTER'; varl: 2));
...
{ * }
function ArrestStringEx(Source: string; SearchAfter, ArrestBefore: Char;
  var ArrestStr: string): string;
function ArrestStringEx2(Source: string; SearchAfter, ArrestBefore: Char;
  var ArrestStr: string): string;
...
function GetFirstWord(Str: string; var sWord: string;
  var FrontSpace: LongInt): string;
...
function FastCharPos(const aSource: string; const c: Char;
...
begin
  ...
  begin
    ...
    begin
      ...
      BitBlt(Canv.Handle, Rect.Left + I * BmWidth, Rect.Top + (J * BmHeight),
        BmWidth, BmHeight, TileImage.Canvas.Handle, 0, 0, SRCCOPY);
...
begin
  ...
  if Assigned(TileImage) then
    ...
    begin
      ...
      JCnt := (Rect.Bottom + BmHeight - 1) div BmHeight -
        (Rect.Top div BmHeight);
      ...
      begin
        ...
        begin
          ...
          BitBlt(Canv.Handle, Rleft, RTop, RWidth, RHeight,
            TileImage.Canvas.Handle, BLeft, btop, SRCCOPY);
...
begin
  ...
  if StartPos > l then
    Exit;
  Dec(StartPos);
  Result := Pos(c, Copy(aSource, StartPos, Length(aSource)));
  (*
    asm
...
begin
  ...
  begin
    ...
    begin
      if Source[I] = SearchAfter then
      begin
        if nArrestBefore = 0 then
        begin
          nSearchAfter := I;
        end;
      end
      else if Source[I] = ArrestBefore then
      begin
        if nSearchAfter > 0 then
        begin
          nArrestBefore := I;
        end;
      end;
      ...
      begin
        ...
        if (nSearchAfter = 1) then
        begin
          Result := Copy(Source, nArrestBefore + 1, srclen - n);
        end
        else
        begin
          Result := Copy(Source, 1, nSearchAfter - 1) + Copy(Source,
            nArrestBefore + 1, srclen);
        end;
        Exit;
      end;
    end;
